...
// Suppress BOOST version complaints
#define BOOST_CONFIG_SUPPRESS_OUTDATED_MESSAGE

#ifndef ES_USE_COMPILER_IMPLEMENTATION
# error ES_USE_COMPILER_IMPLEMENTATION must be defined!
#endif
...
# define ESSCRIPT_COMPILER_TRACE                      ES_DEBUG_TRACE
#else
...
# define ESSCRIPT_COMPILER_TRACE(...)                 ((void)0)
#endif
...
# include "EsScriptCompilerRuleNames.hxx"
  ...
  GRAMMAR_PARSER_DEFINITION_IMPL_BEGIN
    ...
    auto actExternalEnumIdentAdd = [self](EsString::const_pointer start, EsString::const_pointer end) {
      self.m_ctx.externalEnumAdd(
        start,
        end
      );
    };

    auto actExternalObjIdentAdd = [self](EsString::const_pointer start, EsString::const_pointer end) {
      self.m_ctx.externalObjAdd(
        start,
        end
      );
    };
    ...
    //
    enumVal =
      ( ident - self.m_ctx.externalEnums() ) >> 
      root_node_d[OPENUMACC] >>
      ident;
    ...
    stmtConstDecl =
      root_node_d[CONSTANT] >>
        (GRAMMAR_EXPECT(ident) >> no_node_d[ASN] >> expr) >>
    ...
    stmtObjPlainFldDecl =
      (ident-self.m_ctx.reservedWords()) >> ident;

    stmtObjArrayFldDecl =
      (ident-self.m_ctx.reservedWords()) >> ident >> no_node_d[LSBRACKET] >>
    ...
    stmtExternEnumDecl = root_node_d[EXTSYM] >>
      ENUM >>
      GRAMMAR_EXPECT(ident)[ 
        actExternalEnumIdentAdd
      ] >>
      no_node_d[GRAMMAR_EXPECT(SEMICOLON)];
   
    stmtExternObjDecl = root_node_d[EXTSYM] >>
      OBJECT >>
      GRAMMAR_EXPECT(ident)[
        actExternalObjIdentAdd
      ] >>
      no_node_d[GRAMMAR_EXPECT(SEMICOLON)];
...
, 
m_grammar_expr(*this),
m_externalEnums(&m_defaultSymbols),
m_externalObjs(&m_defaultSymbols),
m_dbgLogSuppress(false)
PARSER_CONTEXT_IMPL_INIT_BEGIN

    if(SourceInputType::fromString == inputType)
    {
...
# endif // ES_DUMP_AST_TO_XML
      ...
      {
        std::unique_ptr<EsScriptException> tmp = ES_MAKE_UNIQUE(
          EsScriptException, //< Just create temporaty object instance - it posts an error event, without interrupting compilation thread
          as_<ulong>(EsScriptExceptionCode::EParser),
          getErrorDescr(id),
          debugInfoCreateAt(pos),
          m_errCnt,
  ...
  typedef symbols<int, EsString::value_type> SymbolsT;
  //--------------------------------------------------------------------------------
  
  void externalEnumsSet(SymbolsT* externals)
  {
    m_externalEnums = externals;
  }
  //--------------------------------------------------------------------------------

  void externalObjsSet(SymbolsT* externals)
  {
    m_externalObjs = externals;
  }
  //-------------------------------------------------------------------------------- 

  // Special reserved words singleton specialization
  //
  struct ReservedWords : public SymbolsT
  {
    ReservedWords()
    {
      ...
      for(ulong idx = 0; idx < rw.countGet(); ++idx)
        add(
          rw.nameGet(idx).c_str()
        );
    }
  };

  static const ReservedWords& reservedWords()
  {
    // Init reserved words symbol table once, return singleton reference
    static const ReservedWords sc_reservedWords;

    return sc_reservedWords;
  }
  //--------------------------------------------------------------------------------

  void externalEnumAdd(EsString::const_pointer start, EsString::const_pointer end)
  {
    if( !m_externalEnums )
      return;
  ...
  void externalObjAdd(EsString::const_pointer start, EsString::const_pointer end)
  {
    if(!m_externalObjs)
      return;
  ...
  const SymbolsT& externalEnums() const
  {
    ES_ASSERT(m_externalEnums);

    return *m_externalEnums;
  }
  //--------------------------------------------------------------------------------
  
  const SymbolsT& externalObjs() const
  {
    ES_ASSERT(m_externalObjs);

    return *m_externalObjs;
  }
  //--------------------------------------------------------------------------------
  ...
  static inline bool isKindOfLrhs(long id)
  {
    return identID == id ||
      ...
      objMethodCallID == id ||
      varSvcCallID == id ||
      externEnumValAccID == id ||
      externEnumValLblAccID == id ||
      lrhsID == id;
  }
  //---------------------------------------------------------------------------
  ...
  void compileTo(EsScriptMachine& script)
  {
    ...
    if(SourceInputType::fromSourceFile == m_inputType)
    {
      m_dbgLogSuppress = script.ownerGet().shouldSuppressDbgLog(
        m_file
      );

      ESSCRIPT_COMPILER_TRACE(
        esT("Debug log code generation for %s: suppressed: %d"),
        m_file,
        (int)m_dbgLogSuppress
      );
    }
...
protected:
  ...
  void compileStmt(ParseTreeConstIteratorT cit, EsScriptMachine& script)
  {
    ...
    else if( stmtExternEnumDeclID == id || stmtExternObjDeclID == id )
  ...
  void compileStmtRequire(ParseTreeConstIteratorT cit, EsScriptMachine& script)
  {
    ...
    if( fileNameAbs.empty() ) // throw compiler positional exception
      EsScriptException::Throw(
        as_<ulong>(EsScriptExceptionCode::ESource),
        EsString::format(
          script.loc(),
    ...
    if( isBreaking() )
      EsScriptException::Throw( 
        as_<ulong>(EsScriptExceptionCode::EGeneric),
        esT( "Compilation aborted" ) 
      );
    ...
    fileCompiler.m_externalEnums = m_externalEnums;
    fileCompiler.m_externalObjs = m_externalObjs;
  ...
  {
    ES_ASSERT(stmtExternEnumDeclID == (*cit).value.id().to_long() || stmtExternObjDeclID == (*cit).value.id().to_long());
  ...
  void compileLrhsNested(ParseTreeConstIteratorT cit, EsScriptMachine& script, size_t level)
  {
    ...
    else if( externEnumValAccID == id )
      compileExternEnumValAcc(
        cit,
        script
      );
    else if(externEnumValLblAccID == id)
      compileExternEnumValLblAcc(
        cit,
        script
      );
    else if( lrhsID == id )
  ...
  void compileExternEnumValAcc(ParseTreeConstIteratorT cit, EsScriptMachine& script)
  {
    ES_ASSERT(externEnumValAccID == (*cit).value.id().to_long());
    ES_ASSERT((*cit).children.size() == 2);
    ...
    // Compile and push enuName singleton instance to data stack
    internalCompileEnumInstAcc(
      enuName,
      script,
      debugInfoCreateAt(cit)
    );
    // Push sym name to stack
    m_codeScope->sectionGet()->instructionAdd(
      iLoadConstVal
    ).nameSet(
  ...
  void compileExternEnumValLblAcc(ParseTreeConstIteratorT cit, EsScriptMachine& script)
  {
    ES_ASSERT(externEnumValLblAccID == (*cit).value.id().to_long());
    ES_ASSERT((*cit).children.size() == 2);
  ...
  void compileStmtEnumItemDecl(const EsEnumerationIntf::Ptr& enu, ParseTreeConstIteratorT cit, EsScriptMachine& script)
  {
    ...
    if( val.isEmpty() )
      EsScriptException::Throw(
        as_<ulong>(EsScriptExceptionCode::EEnumDecl),
        EsString::format(
  ...
  // Return false, if code generation was suppressed, true otherwise
  // 
  bool compileExprCall(ParseTreeConstIteratorT cit, EsScriptMachine& script, long explicitCallId = -1)
  {
    long id = static_cast<long>((*cit).value.id().to_long());
    ...
    if(
      ...
      return compileStaticOrBaseCall( //< So far, only explicit static calls to debug script log may be suppressed
        (*cit).children.begin(),
        script
      );
  ...
  // Return false, if code generation was suppressed, true otherwise
  // 
  bool compileStaticOrBaseCall(ParseTreeConstIteratorT cit, EsScriptMachine& script)
  {
    ES_ASSERT(m_codeScope);
    EsScriptCodeSection::Ptr code = m_codeScope->sectionGet();
      ...
      if( !targetNamespace.empty() )
      {
        ...
        else
        {
          ...
          if( info )
          {
            ...
            EsScriptException::ThrowUnknownNamespaceOrService(
              targetNamespace, 
              dbg
            );
        }
    ...
    if(
      m_dbgLogSuppress &&
      esT("EsScriptDebug") == targetNamespace &&
      esT("log") == name
    )
      return false;
  ...
  {
    ES_ASSERT(identID == (*cit).value.id().to_long() || externEnumValAccID == (*cit).value.id().to_long() || externEnumValLblAccID == (*cit).value.id().to_long());
...
protected:
  ...
  // Externals
  SymbolsT* m_externalEnums;
  SymbolsT* m_externalObjs;
  // DebugLog code generator suppression
  bool m_dbgLogSuppress;
...
#if (1 == ES_USE_COMPILER_IMPLEMENTATION)
  EsScriptCompiler::SymbolsT* externalEnums,
  EsScriptCompiler::SymbolsT* externalObjs,
#endif
...
)
{
  ...
  compiler.externalEnumsSet( externalEnums );
  compiler.externalObjsSet( externalObjs );
