...
ES_DECL_REFLECTED_SERVICES_INFO_BEGIN(EsVar, NO_CLASS_DESCR)
  ...
  ES_DECL_REFLECTED_CLASS_METHOD_INFO_STD(EsVar, asStringCollection, EsVariant_ClassCall_cr_EsVariant, NO_METHOD_DESCR)
...
EsVariant EsVar::asStringCollection(const EsVariant& var)
{
  if( var.isObject() )
  {
    EsScriptValAccessorIntf::Ptr acc = var.asObject();
    if( acc )
      return acc->get().asStringCollection();
  }

  return var.asStringCollection();
}
//--------------------------------------------------------------------------------
