...
EsScriptInstructionOpcode EsScriptTryCatchBlock::exec(EsScriptThreadContext& ctx, EsScriptInstructions::const_iterator& instr)
{
  ...
  catch(const EsException& ex)
  {
    ...
    const EsVariant& data = ex.dataGet(); //< If exception was initiated by scripted doThrow - we should have call stack trace data at the end of exception's payload collection
    ...
    m_owner->m_vars.symbolValSetIgnoreReadOnly(
      ...
      EsExceptionInfo::create(
        ex,
        (*instr).debugInfoGet(),
        ( data.isVariantCollection() && data.countGet() && data[data.countGet()-1].isString() ) ?
          data[data.countGet()-1] :
          EsVariant::null()
      )
    );
    result = execCatch(
  ...
  {
    EsVariant trace = ctx.m_csScope->stackTraceGet(true, true, false);
    ctx.m_csScope->stackSnapshotPop();
...
EsMethodInfoKeyT EsScriptCodeSection::keyGet() const ES_NOTHROW 
{ 
  if(m_params)
...
EsScriptCodeSection::~EsScriptCodeSection()
{
...
EsString::Array EsScriptCodeSection::paramsTraceGet() const ES_NOTHROW
{
  const EsString::Array& names = paramNamesAccess();
  ...
  for(auto const& name : names)
  {
    try
    {
      EsScriptValAccessorIntf::Ptr paramVar = variableGet(
        name
      );

      result.push_back(
        name + esT(":") + paramVar->trace()
      );
    }
    catch(...) //< We do not throw by this method's design
    {
    }
  }
...
EsString::Array EsScriptCodeSection::localsTraceGet() const ES_NOTHROW
{
  const EsString::Array& names = m_vars.allSymbolNamesGet();
  const EsString::Array& params = paramNamesAccess();
  ...
  for(auto const& name : names)
  {
    ...
    try
    {
      EsScriptValAccessorIntf::Ptr var = variableGet(
        name
      );
