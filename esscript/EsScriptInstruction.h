...
public:
  ...
  /// Short-cuts for instruction location access, if debug info is set.
  /// If not, (ulong)(-1) is returned
  ///
  inline ulong lineGet() const ES_NOTHROW
  {
    if( m_debugInfo )
      return m_debugInfo->lineGet();

    return static_cast<ulong>(-1);
  }

  inline ulong columnGet() const ES_NOTHROW
  {
    if( m_debugInfo )
      return m_debugInfo->columnGet();

    return static_cast<ulong>(-1);
  }

  /// Return optional source file for the current instruction.
  /// If debug info is not present, return null string
  ///
  inline EsString fileGet() const ES_NOTHROW
  {
    if( m_debugInfo )
      return m_debugInfo->fileGet();
