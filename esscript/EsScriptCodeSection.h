...
public:
  ...
  // get code section method key
  EsMethodInfoKeyT keyGet() const ES_NOTHROW;
  ...
  inline EsScriptValAccessorIntf::Ptr variableGet(const EsString& name) const
  {
    return m_vars.symbolGet(
      name, 
      true
    );
  }
  ...
  /// Return input parameters trace
  EsString::Array paramsTraceGet() const ES_NOTHROW;

  /// Return local variables trace
  EsString::Array localsTraceGet() const ES_NOTHROW;
  ...
  void checkTemplateOperation() const
  {
    if( !m_template )
      EsScriptException::Throw(
        ...
        esT("Operation is supported only by template code sections")
      );
  }
