...
class EsScriptObjectCtorAcc : public EsScriptObject
{
public:
  EsScriptObjectCtorAcc(
...
) const
{
  ...
  std::unique_ptr<EsScriptObjectCtorAcc> result = ES_MAKE_UNIQUE(
    EsScriptObjectCtorAcc,
    m_typeName,
    m_ancestor ?
...
)
{
  ...
  if( field->isConditional() )
    EsScriptException::Throw(
      ...
      esT("Conditional fields must be added using specific 'fieldConditionalAdd' interface"),
      dbg
    );
...
ES_IMPL_INTF_METHOD(EsScriptObjectIntf::Ptr, EsScriptObject::clone)() const
{
  ...
  if( !isMetaclass() && !isRoot() )
    EsScriptException::Throw(
      as_<ulong>(EsScriptExceptionCode::EMethodDecl),
  ...
  const EsString& readerName, const EsString& writerName)
{
  ...
  if( isPOD() || isArray() || isIf() )
    EsScriptException::ThrowPodObjectMayNotContainFieldsVarsOrProps(
      typeName, 
      currentDebugInfoGet()
    );
  ...
  if( ffinder.found() )
    EsScriptException::ThrowPropertyAlreadyDeclared(
      name, 
      typeName, 
      currentDebugInfoGet()
    );
...
EsString EsScriptObject::propertyLabelGet(cr_EsString propName) const
{
  ...
  if( prop )
  {
    if( prop->hasAttribute(esT("label")) )
      return esTranslationGet( prop->attributeGet(esT("label")).asString() );
  }
  else
  {
    const EsPropertyInfo* info = classInfoGet().propertyInfoFind(propName);
...
EsString EsScriptObject::propertyDescriptionGet(cr_EsString propName) const
{
  ...
  if( prop )
  {
    if( prop->hasAttribute(esT("help")) )
      return esTranslationGet( prop->attributeGet(esT("help")).asString() );
  }
  else
  {
    const EsPropertyInfo* info = classInfoGet().propertyInfoFind(propName);
...
EsScriptDebugInfoIntf::Ptr EsScriptObject::currentDebugInfoGet() const ES_NOTHROW
{
  if(m_ctx)
  {
    EsScriptMachine* machine = m_ctx->vm();
    if( machine )
      return machine->currentDebugInfoGet();
  }
