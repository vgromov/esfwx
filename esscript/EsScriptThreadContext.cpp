...
void EsScriptThreadContext::methodSetCurrent(const EsMethodInfoKeyT& key)
{
  ...
  if(!result)
    EsScriptException::Throw(
      as_<ulong>(EsScriptExceptionCode::EExecution),
      EsString::format(
...
)
{
  ...
  catch(const std::exception& ex)
  {
    ...
    EsVariant trace = m_csScope->stackTraceGet(true, true, false);
...
void EsScriptThreadContext::doConstructMemberAccessor(const EsScriptInstruction& instr)
{
  ...
  if(!obj)
    EsScriptException::Throw(
      ...
      EsString::format(
        esT("Error accessing object field or member variable '%s'. Either object instance does not exist, or member is not declared."),
        name
      ),
      instr.debugInfoGet()
    );
...
void EsScriptThreadContext::doAccessAttributeValue(const EsScriptInstruction& instr)
{
  ...
  if(obj)
  {
    if(iLoadThisAttributeVal == instr.opcode())
    {
      // try active code section attributes first, if This is implied
      if( currentMethodGet()->attributesAccess()->attributeExists(name) )
        acc = EsScriptTmpValAccessor::create(
          currentMethodGet()->attributesAccess()->attributeGet(name)
        );
      else //< Next, try property, if it's the case
      {
        const EsMethodInfoKeyT& key = currentMethodGet()->keyGet();
        EsString potentialPropertyName;
        const EsString& curMethodName = key.nameGet();

        // Check for scripted properties
        if(1 == key.parametersCountGet() && ((curMethodName.size()-6) == curMethodName.find(esT("_write"))) )
          potentialPropertyName = curMethodName.substr(
            0, 
            curMethodName.size()-6
          );
        else if(0 == key.parametersCountGet() && ((curMethodName.size() - 5) == curMethodName.find(esT("_read"))) )
          potentialPropertyName = curMethodName.substr(
            0,
            curMethodName.size() - 5
          );

        if(!potentialPropertyName.empty())
        {
          EsScriptObject* so = ES_INTFPTR_TO_OBJECTPTR<EsScriptObject, EsScriptObjectIntf::Ptr>(obj);
          ES_ASSERT(so);

          if(
            so->thisHasScriptedProperty(potentialPropertyName) &&
            so->propertyHasAttribute(potentialPropertyName, name)
          )
          {
            acc = EsScriptTmpValAccessor::create(
              so->propertyAttributeGet(
    ...
    if( !acc ) //< Fallback to object attribute
      acc = EsScriptTmpValAccessor::create(
...
)
{
  ...
  catch(const EsException& ex)
  {
    ...
    EsScriptException::ReThrow(
      ex,
      dbg,
      dbg //< Log re-thrown error as well, as it may provide error location info this time
    );
  }
}
//---------------------------------------------------------------------------
...
)
{
  ...
  obj->typeNameGet().hashGet(); //< Force one-time type name string hash calculation
...
void EsScriptThreadContext::doThrow(const EsScriptInstruction& instr)
{
  ...
  if(iThrow == instr.opcode())
  {
    ...
    EsVariant payload = m_csScope->stackPop()->get(); //< Copy to local var - prevent loosing our payload when context's data stack goes out of scope
    const EsVariant& trace = m_csScope->stackTraceGet( //< Capture current stack trace, except for detailed data stack
      true, 
      true, 
      false
    );

    // Append exception stack trace to an end of payload
    if( payload.isEmpty() )
    {
      payload = EsVariantArray();
      payload.addToVariantCollection(
        EsVariant::null()
      );
    }
    else
      payload = payload.asVariantCollection();

    payload.addToVariantCollection( trace );

    // By convention, if code throws collection, and its first element is numeric,
    // we consider it as an error code of sorts.
    // The same result would be achieved if all payload is single numeric.
    //
    ulong code = as_<ulong>(EsScriptExceptionCode::EUnspecified);
    EsString reason = _("Error reason is unspecified");
    bool payloadAttach = true;
    if( payload.isCollection() && payload.countGet() > 0 )
    {
      if( payload[0].isNumeric() )
      {
        code = payload[0].asULong();
        if( payload.countGet() > 1 )
          reason = payload[1].asString();
      }
      else if(payload[0].isString())
        reason = payload[0].asString();
    }
    else if( payload.isNumeric() )
    {
      code = payload.asULong();
      payloadAttach = false;
    }
    else if( payload.isString() )
    {
      reason = payload.asString();
      payloadAttach = false;
    }
...
)
{
  ...
  catch(const EsException& ex)
  {
    ...
    EsScriptException::ReThrow(
      ex,
      dbg,
      dbg //< Log this error if it has debug info set
    );
  }
}
//---------------------------------------------------------------------------
...
EsString EsScriptCodeSectionScope::scopeTraceGet(bool includeParams, bool includeLocals, bool includeDataStack) const ES_NOTHROW
{
  EsString trace;

  trace = esT("Caller: ") + m_code->nameGet();

  if( includeParams )
    trace += esT("; Params: ") + EsString::fromStringArray( m_code->paramsTraceGet() );

  if( includeLocals )
    trace += esT("; Locals: ") + EsString::fromStringArray( m_code->localsTraceGet() );

  if( includeDataStack  )
    trace += esT("; Data: ") + EsString::fromStringArray( dataStackTraceGet() );

  return trace;
}
//--------------------------------------------------------------------------------

EsString::Array EsScriptCodeSectionScope::stackTraceGet(bool includeParams, bool includeLocals, bool includeDataStack) const ES_NOTHROW
{
  const EsScriptCodeSectionScope* scope = this;
  ...
  while(scope)
  {
    ...
    scope = scope->parentScopeGet();
  }

  ES_DEBUG_TRACE(
    esT("EsScriptCodeSectionScope::stackTraceGet:")
  );
#ifdef ES_DEBUG
...
EsString::Array EsScriptCodeSectionScope::dataStackTraceGet() const ES_NOTHROW
{
  return m_stack.trace();
}
//---------------------------------------------------------------------------
