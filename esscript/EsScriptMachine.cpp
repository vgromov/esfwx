...
EsScriptContext::~EsScriptContext()
{
  unlink();
  while( !m_links.empty() )
  ...
  else
  {
    ...
    if( EsScriptSymbolFlag::ExternEnum != (flags & EsScriptSymbolFlag::ExternEnum) )
      EsScriptException::Throw(
        as_<ulong>(EsScriptExceptionCode::EGenericDecl),
        EsString::format(
...
# endif
    ...
    {
      EsScriptException::Throw(
        as_<ulong>(EsScriptExceptionCode::EBinary),
        EsString::format(
...
EsVariant EsScriptMachine::enumValueGet(const EsString& name, const EsString& sym) const
{
...
EsVariant EsScriptMachine::enumValueLabelGet(const EsString& name, const EsString& sym) const
{
  EsReflectedClassIntf::Ptr enu = enumFind(name, true);
  if(enu->call(esT("hasSymbol"), sym).asBool())
...
EsScriptObjectIntf::Ptr EsScriptMachine::metaclassDeclare(const EsString &name, const EsString &baseName)
{
  ...
  std::unique_ptr<EsScriptObjectCtorAcc> ptr = ES_MAKE_UNIQUE(
    EsScriptObjectCtorAcc,
    name,
    baseMetaclass,
    methods,
