...
public:
  ...
  {
    std::unique_ptr<EsScriptValAccessor> ptr = ES_MAKE_UNIQUE(
      EsScriptValAccessor,
      name,
      var,
      flags,
...
void EsScriptSymbolTable::checkTemplateOperation() const
{
  if( m_checkTemplate && !m_template )
    EsScriptException::Throw(
      ...
      esT("Operation only allowed on symbol table template")
    );
}
...
void EsScriptSymbolTable::checkNonTemplateOperation() const
{
  if( m_checkTemplate && m_template )
    EsScriptException::Throw(
      as_<ulong>(EsScriptExceptionCode::EOperation),
      esT("Operation only allowed on non template symbol table")
    );
}
