...
ES_DECL_INTF_BEGIN(D558F74B, 4D8A4b21, A441FB9E, D5F74C32, EsScriptIntf)
  ...
  /// Access script enumeration object by its name
  ES_DECL_INTF_METHOD(EsVariant, enumerationGet)(cr_EsString) const = 0;
  ...
  ///
  ES_DECL_INTF_METHOD(EsVariant, enumerationEval)(cr_EsString) const = 0;
  ...
  /// Return this script's metaclass object
  ES_DECL_INTF_METHOD(EsMetaclassIntf::Ptr, metaclassGet)() const = 0;
...
/// ES script class implementation
///
...
public:
  ...
  // Access script parameters as a whole
  // 
  const EsScriptParameters& parametersGet() const ES_NOTHROW { return m_params; }
  void parametersSet(const EsScriptParameters& params) ES_NOTHROW { m_params = params; }
  ...
  ES_DECL_INTF_METHOD(const EsStringArray&, includePathsGet)() const ES_OVERRIDE { return m_params.m_includes; }
  ...
  ES_DECL_INTF_METHOD(const EsStringArray&, linkPathsGet)() const ES_OVERRIDE { return m_params.m_linkPaths; }
  ...
  /// Access script enumeration object by its name
  ES_DECL_REFLECTED_INTF_CONST_METHOD1(EsVariant, enumerationGet, cr_EsString) ES_OVERRIDE;

  /// Evaluate enum value or lable by enum_name$$value_name|enum_name$$value_name$$label expression.
  /// Throw an exception if either enum or symbol is not found
  ///
  ES_DECL_REFLECTED_INTF_CONST_METHOD1(EsVariant, enumerationEval, cr_EsString) ES_OVERRIDE;
  ...
  /// Return this script's metaclass object
  ES_DECL_INTF_METHOD(EsMetaclassIntf::Ptr, metaclassGet)() const ES_OVERRIDE { return get_meta(); }
  ...
  ES_DECL_PROPERTY(translatablesGenerate, bool)
  ES_DECL_PROPERTY(debugLogSuppress, bool)
  ES_DECL_PROPERTY(debugLogSuppressInclFirst, bool)
  ES_DECL_PROPERTY(debugLogSuppressIncl, EsStringArray)
  ES_DECL_PROPERTY(debugLogSuppressExcl, EsStringArray)

  /// @brief  Determine if we should suppress debug log while compiling the file at specified path
  /// @param [in] file  Full pathname of the file.
  /// @returns  True if calls to EsScriptDebug::log should generate code, false otherwise.
  ///
  bool shouldSuppressDbgLog(const EsString& file) const;
  
  // Static services
  //
  static bool shouldSuppressDbgLog(const EsString& file, bool inclFirst, const EsString::Array& incl, const EsString::Array& excl);
...
protected:
  ...
  ES_NO_DEFAULT_CTR(EsScript);
  ES_NON_COPYABLE(EsScript);
};
