...
class EsEventHandlerCtorAcc : public EsEventHandler
{
public:
  EsEventHandlerCtorAcc(bool sync, const EsString& categories) :
  EsEventHandler(
    sync,
    categories
  )
...
EsEventHandler::EsEventHandler(bool sync, const EsString& categories) :
m_isSync(sync),
m_sub(nullptr)
{
  m_dynamic = true;
...
{
  std::unique_ptr<EsEventHandlerCtorAcc> tmp = ES_MAKE_UNIQUE(
    EsEventHandlerCtorAcc,
    true,
    EsString::null()
  );
  ES_ASSERT(tmp);
