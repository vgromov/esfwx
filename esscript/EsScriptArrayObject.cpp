...
class EsScriptArrayObjectCtorAcc : public EsScriptArrayObject
{
public:
  EsScriptArrayObjectCtorAcc(const EsScriptContext::Ptr& ctx, esU32 flags, const EsScriptObjectDataBufferPtr& buff,
...
EsScriptObjectIntf::Ptr EsScriptArrayObject::createMetaclass(const EsScriptContext::Ptr& ctx, const EsScriptObjectIntf::Ptr& itemMetaclass)
{
  ...
  std::unique_ptr<EsScriptArrayObjectCtorAcc> tmp = ES_MAKE_UNIQUE(
    EsScriptArrayObjectCtorAcc,
    ctx,
...
{
  std::unique_ptr<EsScriptArrayObjectCtorAcc> result = ES_MAKE_UNIQUE(
    EsScriptArrayObjectCtorAcc,
    m_ctx,
