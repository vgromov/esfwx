...
public:
  ...
  {
    EsScriptMethodMapPtr scopedMethods = ES_MAKE_SHARED( EsScriptMethodMap );
    std::unique_ptr<EsScriptIfBranchObject> ptr = ES_MAKE_UNIQUE(
      EsScriptIfBranchObject,
      ctx,
...
class EsScriptIfObjectCtorAcc : public EsScriptIfObject
{
public:
  EsScriptIfObjectCtorAcc(const EsScriptContext::Ptr& ctx, esU32 flags, const EsScriptObjectDataBufferPtr& buff, const EsScriptCodeSection::Ptr& expr) :
  EsScriptIfObject(
    ctx,
    flags,
    buff,
...
EsScriptObjectIntf::Ptr EsScriptIfObject::createMetaclass(const EsScriptContext::Ptr& ctx)
{
  ...
  std::unique_ptr<EsScriptIfObjectCtorAcc> tmp = ES_MAKE_UNIQUE(
    EsScriptIfObjectCtorAcc,
    ctx,
    ofMetaclass|ofIf,
    nullptr,
    nullptr
...
{
  std::unique_ptr<EsScriptIfObjectCtorAcc> result = ES_MAKE_UNIQUE(
    EsScriptIfObjectCtorAcc,
    m_ctx,
    (m_flags & ~ofMetaclass) | ofNeedUpdateLayout,
    buff,
