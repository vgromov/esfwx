...
# endif
  ...
  virtual bool isOk() const ES_OVERRIDE
  {
    return !m_sock.empty();
  }

  virtual bool bind(bool doThrow) ES_OVERRIDE
  {
    m_error = 0;
    if( !m_sock.empty() )
    {
...
# endif
  ...
  virtual bool listen(int backlog, bool doThrow) ES_OVERRIDE
  {
    m_error = 0;
    if( !m_sock.empty() )
    {
...
# endif
  ...
  virtual EsSocket::Impl* accept(ulong tmo, bool doThrow) ES_OVERRIDE
  {
    m_error = 0;
    if( !m_sock.empty() )
    {
...
# endif
  ...
  virtual bool connect(bool doThrow) ES_OVERRIDE
  {
    m_error = 0;
    if( !m_sock.empty() )
    {
  ...
  virtual bool shutdown(EsSocketShutdownMode mode, bool doThrow) ES_OVERRIDE
  {
    m_error = 0;
    if( !m_sock.empty() )
    {
  ...
  virtual void close(bool doThrow) ES_OVERRIDE
  {
...
# endif
  ...
  virtual ulong receive(esU8* data, ulong len, ulong tmo, bool doThrow) ES_OVERRIDE
  {
    m_error = 0;
    esU8* pos = data;
    if( !m_sock.empty() && len )
  ...
  virtual ulong receiveFrom(EsSocketAddr& addr, esU8* data, ulong len, ulong tmo, bool doThrow) ES_OVERRIDE
  {
    m_error = 0;
    if( !m_sock.empty() && len )
    {
...
# endif
  ...
  virtual ulong incomingSizeGet(ulong tmo, bool doThrow) ES_OVERRIDE
  {
#ifdef ES_COMM_USE_BLUETOOTH
    if( EsSocketType::Bluetooth == m_type )
    {
