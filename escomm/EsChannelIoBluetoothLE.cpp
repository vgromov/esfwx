...
{
  std::unique_ptr<EsChannelIoBluetoothLE> chnl = ES_MAKE_UNIQUE(EsChannelIoBluetoothLE);
  ES_ASSERT(chnl);
  chnl->m_dynamic = true;
...
void EsChannelIoBluetoothLE::init()
{
  ...
  EsVariant::Array vradios = EsBluetoothRadioEnumerator::enumerate().asVariantCollection();

  for( auto const& vradio : vradios )
  {
    EsBluetoothRadio* pradio = ES_VARIANT_TO_OBJECTPTR<EsBluetoothRadio>(vradio);
    ES_ASSERT(pradio);

    if( pradio->get_enabled() )
    {
      EsBluetoothLEdeviceEnumerator leEnum( vradio.asExistingObject() );

      leEnum.enumerate(true, 5000);
      EsVariant::Array vdevs = leEnum.get_devices().asVariantCollection();

      for(auto const& vdev : vdevs)
      {
        EsBluetoothLEdevice* pdev = ES_VARIANT_TO_OBJECTPTR<EsBluetoothLEdevice>(vdev);
        ES_ASSERT(pdev);

        if( m_addr == pdev->get_address() )
        {
          m_dev = vdev.asExistingObject();
          pdev->discover();

          EsBluetoothLEservice* svc = pdev->serviceFind(
            byteSvcUUID()
          );
          ...
          if( svc )
          {
            ...
            {
              m_name = pdev->get_name();
              m_radio = vradio.asExistingObject();
              return;
            }
          }
        }
...
void EsChannelIoBluetoothLE::uninit()
{
  ...
  EsBluetoothLEdevice* pdev = ES_INTFPTR_TO_OBJECTPTR<EsBluetoothLEdevice>(m_dev);
  if( 
    pdev && 
    pdev->isConnected() 
  )
...
bool EsChannelIoBluetoothLE::internalIsConnected() const
{
  return internalIsOpen() &&
    ES_INTFPTR_TO_OBJECTPTR<EsBluetoothLEdevice>(m_dev)->isConnected();
}
//---------------------------------------------------------------------------
...
void EsChannelIoBluetoothLE::internalConnect()
{
  ...
  if( EsBluetoothLEresult::gattSuccess == m_result )
  {
    ...
    if( EsBluetoothLEresult::gattSuccess == m_result )
    {
      ...
      EsBluetoothLEdescriptor* dcr = m_chxPendingBytes->descriptorFind(
        EsBluetoothLEdescriptor::uuidGetByKind(
          EsBluetoothLEdescriptorKind::ClientConfiguration
        )
      );
...
esU32 EsChannelIoBluetoothLE::internalBytesGet(esU8* dest, esU32 toRead, esU32 tmo)
{
  ...
  )
  {
    while( toRead && tmo && !internalIsBreaking() )
    {
      ...
      if( EsBluetoothLEresult::gattSuccess == m_result )
      {
        esU32 total = static_cast<esU32>(m_cacheRx.size());
  ...
  return static_cast<esU32>(pos-dest);
}
//---------------------------------------------------------------------------
...
esU32 EsChannelIoBluetoothLE::txCacheSend()
{
  ...
  esU32 toWrite = static_cast<esU32>(m_cacheTx.size());
  ...
  return static_cast<esU32>(pos-src);
}
//---------------------------------------------------------------------------
