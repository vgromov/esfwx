...
void EsSocketServer::Listener::onExitWorker()
{
  ...
  // Finally, close listening socket itself
  EsCriticalSectionLocker lock(m_owner.m_cs);
  if( 
    m_owner.m_impl && 
    m_owner.m_impl->isBound() && 
    m_owner.m_impl->isListening() 
  )
    m_owner.m_impl->close(false);
}
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
...
{
  std::unique_ptr<EsSocketServerReflected> p = ES_MAKE_UNIQUE(EsSocketServerReflected);
  ES_ASSERT(p);
