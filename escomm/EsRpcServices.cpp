...
ES_DECL_ENUMERATION_INFO(EsRpcStatus, esT("RPC execution status codes"))
// ----------------------------------------------------------------------
...
)
{
  const EsVariant::Array& payload = {
    static_cast<ulong>(stat),
    static_cast<ulong>(sig),
    rpcId,
    ioErr
  };
...
ES_DECL_BASE_CLASS_INFO_BEGIN(EsRpcMaster, esT("RPC master services over Ekosfera protocol."))
  ...
  ES_DECL_PROP_INFO(                  EsRpcMaster, active, bool, _i("Master is active"), NO_DEFAULT_VAL, NO_PROPERTY_DESCR)
...
{
  std::unique_ptr<EsRpcMaster> master = ES_MAKE_UNIQUE( EsRpcMaster );
  ES_ASSERT( master );
...
bool EsRpcMaster::get_active() const
{
  return isActive();
}
//--------------------------------------------------------------------------------

void EsRpcMaster::set_active(const bool& active)
{
  if( isActive() == active )
    return;
