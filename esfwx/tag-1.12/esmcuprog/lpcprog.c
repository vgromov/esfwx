...
{
    { 0, 0, 0, 0, 0, 0, 0, CHIP_VARIANT_LPC2XXX },  /* unknown */
...
static void PhilipsOutputErrorMessage(ISP_ENVIRONMENT* env, unsigned char ErrorNumber)
{
    switch (ErrorNumber)
    {
    case   0:
        DebugPrintf(env, 2, "CMD_SUCCESS");
        break;

    case   1:
        DebugPrintf(env, 1, "INVALID_COMMAND");
        break;

    case   2:
        DebugPrintf(env, 1, "SRC_ADDR_ERROR: Source address is not on word boundary.");
        break;
    case   3:
        DebugPrintf(env, 1, "DST_ADDR_ERROR: Destination address is not on a correct boundary.");
        break;

    case   4:
        DebugPrintf(env, 1, "SRC_ADDR_NOT_MAPPED: Source address is not mapped in the memory map."
                       "                     Count value is taken into consideration where applicable.");
        break;

    case   5:
        DebugPrintf(env, 1, "DST_ADDR_NOT_MAPPED: Destination address is not mapped in the memory map."
                       "                     Count value is taken into consideration where applicable.");
        break;

    case   6:
        DebugPrintf(env, 1, "COUNT_ERROR: Byte count is not multiple of 4 or is not a permitted value.");
        break;

    case   7:
        DebugPrintf(env, 1, "INVALID_SECTOR: Sector number is invalid or end sector number is"
                       "                greater than start sector number.");
        break;

    case   8:
        DebugPrintf(env, 1, "SECTOR_NOT_BLANK");
        break;

    case   9:
        DebugPrintf(env, 1, "SECTOR_NOT_PREPARED_FOR_WRITE_OPERATION:"
                       "Command to prepare sector for write operation was not executed.");
        break;

    case  10:
        DebugPrintf(env, 1, "COMPARE_ERROR: Source and destination data not equal.");
        break;

    case  11:
        DebugPrintf(env, 1, "BUSY: Flash programming hardware interface is busy.");
        break;

    case  12:
        DebugPrintf(env, 1, "PARAM_ERROR: Insufficient number of parameters or invalid parameter.");
        break;

    case  13:
        DebugPrintf(env, 1, "ADDR_ERROR: Address is not on word boundary.");
        break;

    case  14:
        DebugPrintf(env, 1, "ADDR_NOT_MAPPED: Address is not mapped in the memory map."
                       "                 Count value is taken in to consideration where applicable.");
        break;

    case  15:
        DebugPrintf(env, 1, "CMD_LOCKED");
        break;

    case  16:
        DebugPrintf(env, 1, "INVALID_CODE: Unlock code is invalid.");
        break;

    case  17:
        DebugPrintf(env, 1, "INVALID_BAUD_RATE: Invalid baud rate setting.");
        break;

    case  18:
        DebugPrintf(env, 1, "INVALID_STOP_BIT: Invalid stop bit setting.");
        break;

    case  19:
        DebugPrintf(env, 1, "CODE READ PROTECTION ENABLED");
        break;
    ...
    default:
        DebugPrintf(env, 1, "unknown error %u", ErrorNumber);
        break;
    }
...
*/
static unsigned char GetAndReportErrorNumber(ISP_ENVIRONMENT* env, const char *Answer)
{
    ...
    while (1)
    {
        ...
        if (Answer[i] == 0x0a)
        {
            ...
            {
                DebugPrintf(env, 1, "ErrorString: %s", &Answer[i]);
                break;
            }
    ...
    PhilipsOutputErrorMessage(env, Result);
...
//    char * cmdstr;
    ...
    if (!IspEnvironment->DetectOnly)
    {
        ...
        if(LPCtypes[IspEnvironment->DetectedDevice].ChipVariant == CHIP_VARIANT_LPC2XXX)
        {
            ...
            * again. The resulting vector table should checksum to 0. */
            ...
            DebugPrintf(IspEnvironment, 3, "Position 0x14 patched: ivt_CRC = 0x%08lX\n", ivt_CRC);
        }
                ...
                LPCtypes[IspEnvironment->DetectedDevice].ChipVariant == CHIP_VARIANT_LPC11XX)
        {
            ...
            * again. The resulting vector table should checksum to 0. */
            ...
            DebugPrintf(IspEnvironment, 3, "Position 0x1C patched: ivt_CRC = 0x%08lX\n", ivt_CRC);
        }
    }

    DebugPrintf(IspEnvironment, 2, "Synchronizing (ESC to abort)");
...
#endif
    {
        for (nQuestionMarks = found = 0; !found && nQuestionMarks < IspEnvironment->nQuestionMarks; nQuestionMarks++)
        {
            ...
            if (!AppSyncing(IspEnvironment, nQuestionMarks))
            {
            ...
            DumpString(IspEnvironment, 3, strippedAnswer, strippedsize, tmp_string);
            ...
            if (strcmp(strippedAnswer, "Bootloader\r\n") == 0 && IspEnvironment->TerminalOnly == 0)
            {
                ...
                DebugPrintf(IspEnvironment, 2, "Entering ISP; re-synchronizing (watchdog = %ld seconds)", WatchDogSeconds);
                sprintf(temp, "T %lu\r\n", ticks);
                ...
                {
                    ResetKeyboardTtySettings();
                    DebugPrintf(IspEnvironment, 2, "No answer on 'watchdog timer set'");
    ...
    if (!found)
    {
        DebugPrintf(IspEnvironment, 1, " no answer on '?'");
        return (NO_ANSWER_QM);
    }

#if defined INTEGRATED_IN_WIN_APP
    AppSyncing(IspEnvironment, -1);                         // flag syncing done
#endif

    DebugPrintf(IspEnvironment, 2, " OK");
    ...
    {
        DebugPrintf(IspEnvironment, 1, "No answer on 'Synchronized'");
        return (NO_ANSWER_SYNC);
    }

    DebugPrintf(IspEnvironment, 3, "Synchronized 1");

    DebugPrintf(IspEnvironment, 3, "Setting oscillator");
    ...
    {
        DebugPrintf(IspEnvironment, 1, "No answer on Oscillator-Command");
        return (NO_ANSWER_OSC);
    }

    DebugPrintf(IspEnvironment, 3, "Unlock");

    cmdstr = "U 23130\n";

    if (!SendAndVerify(IspEnvironment, cmdstr, Answer, sizeof Answer))
    {
        DebugPrintf(IspEnvironment, 1, "Unlock-Command:");
        return (UNLOCK_ERROR + GetAndReportErrorNumber(IspEnvironment,Answer));
    }

    DebugPrintf(IspEnvironment, 2, "Read bootcode version: ");
    ...
    {
        DebugPrintf(IspEnvironment, 1, "no answer on Read Boot Code Version");
    ...
    if (strncmp(Answer + strlen(cmdstr), "0\r\n", 3) == 0)
    {
        ...
        DebugPrintf(2, "%d.%d.%d\n", maj, min, build);
        */
        DebugPrintf(IspEnvironment, 2, strippedAnswer);
    }
    else
    {
        DebugPrintf(IspEnvironment, 2, "unknown");
    }

    DebugPrintf(IspEnvironment, 2, "Read part ID: ");
    ...
    if (strncmp(Answer, cmdstr, strlen(cmdstr)) != 0)
    {
        DebugPrintf(IspEnvironment, 1, "no answer on Read Part Id");
    ...
    *endPtr = '\0'; /* delete \r\n */
    ...
    if (IspEnvironment->DetectedDevice == 0) {
        ...
        DebugPrintf(IspEnvironment, 2, "LPC%s, %d kiB ROM / %d kiB SRAM",
    ...
    DebugPrintf(IspEnvironment, 2, " (0x%X)", Pos);//strippedAnswer);
    ...
    */
    ...
    DebugPrintf(IspEnvironment, 2, "Will start programming at Sector 1 if possible, and conclude with Sector 0 to ensure that checksum is written last.");
    ...
    {
        DebugPrintf(IspEnvironment, 2, "Wiping Device. ");
        ...
        {
            DebugPrintf(IspEnvironment, 1, "Wrong answer on Prepare-Command");
            return (WRONG_ANSWER_PREP + GetAndReportErrorNumber(IspEnvironment,Answer));
        }
        ...
        if (!SendAndVerify(IspEnvironment, tmpString, Answer, sizeof Answer))
        {
            DebugPrintf(IspEnvironment, 1, "Wrong answer on Erase-Command");
            return (WRONG_ANSWER_ERAS + GetAndReportErrorNumber(IspEnvironment, Answer));
        }
        DebugPrintf(IspEnvironment, 2, "OK ");
    }
    else{
        //no wiping requested: erasing sector 0 first
        DebugPrintf(IspEnvironment, 2, "Erasing sector 0 first, to invalidate checksum. ");
    ...
    while (1)
    {
        ...
        {
            DebugPrintf(IspEnvironment, 1, "Program too large; running out of Flash sectors.\n");
            return (PROGRAM_TOO_LARGE);
        }

        DebugPrintf(IspEnvironment, 2, "Sector %ld: ", Sector);
        ...
        if (IspEnvironment->BinaryOffset < ReturnValueLpcRamStart(IspEnvironment)) // Skip Erase when running from RAM
        {
            ...
            if (!SendAndVerify(IspEnvironment, tmpString, Answer, sizeof Answer))
            {
                DebugPrintf(IspEnvironment, 1, "Wrong answer on Prepare-Command (1) (Sector %ld)", Sector);
                return (WRONG_ANSWER_PREP + GetAndReportErrorNumber(IspEnvironment, Answer));
            }
            ...
            if (IspEnvironment->WipeDevice == 0 && (Sector!=0)) //Sector 0 already erased
            {
                ...
                if (!SendAndVerify(IspEnvironment, tmpString, Answer, sizeof Answer))
                {
                    DebugPrintf(IspEnvironment, 1, "Wrong answer on Erase-Command (Sector %ld)", Sector);
                    return (WRONG_ANSWER_ERAS + GetAndReportErrorNumber(IspEnvironment,Answer));
                }
        ...
        for (SectorOffset = 0; SectorOffset < SectorLength; SectorOffset += SectorChunk)
        {
            if (SectorOffset > 0)
            {
                ...
                DebugPrintf(IspEnvironment, 2, "|");  /* means: partial segment copied */
                fflush(stdout);
            }
            ...
            if (!SendAndVerify(IspEnvironment, tmpString, Answer, sizeof Answer))
            {
                DebugPrintf(IspEnvironment, 1, "Wrong answer on Write-Command");
                return (WRONG_ANSWER_WRIT + GetAndReportErrorNumber(IspEnvironment,Answer));
            }
                ...
                for (Block = 0; Block < 4; Block++)  // Each block 45 bytes
                {
                    ...
                    for (BlockOffset = 0; BlockOffset < 45; BlockOffset++)
                    {
                        ...
                        { // Flash: use full memory
													tmpPos = Pos + Block * 45 + BlockOffset;
                        }
                        else
                        { // RAM: Skip first 0x200 bytes, these are used by the download program in LPC21xx
                          tmpPos = Pos + Block * 45 + BlockOffset + 0x200;
                        }
                        
                        // check for proper mem access restrictions!!!
                        if( tmpPos < IspEnvironment->BinaryLength )
													c = IspEnvironment->BinaryContent[tmpPos];                        
												else 
													c= 0;
                    ...
                    DebugPrintf(IspEnvironment, 3, "Line = %d", Line);
                    ...
                    if (Line == 20)
                    {
                        ...
                        if (repeat >= 3)
                        {
                            DebugPrintf(IspEnvironment, 1, "Error on writing block_CRC (1)");
            ...
            if (Line != 0)
            {
                ...
                if (repeat >= 3)
                {
                    DebugPrintf(IspEnvironment, 1, "Error on writing block_CRC (3)");
            ...
            if (IspEnvironment->BinaryOffset < ReturnValueLpcRamStart(IspEnvironment))
            {
                ...
                if (!SendAndVerify(IspEnvironment, tmpString, Answer, sizeof Answer))
                {
                    DebugPrintf(IspEnvironment, 1, "Wrong answer on Prepare-Command (2) (Sector %ld)", Sector);
                    return (WRONG_ANSWER_PREP2 + GetAndReportErrorNumber(IspEnvironment,Answer));
                }
                ...
                if (!SendAndVerify(IspEnvironment, tmpString, Answer, sizeof Answer))
                {
                    DebugPrintf(IspEnvironment, 1, "Wrong answer on Copy-Command");
                    return (WRONG_ANSWER_COPY + GetAndReportErrorNumber(IspEnvironment,Answer));
                }
                ...
                if (IspEnvironment->Verify)
                {
                    ...
                    if (!SendAndVerify(IspEnvironment, tmpString, Answer, sizeof Answer))
                    {
                        DebugPrintf(IspEnvironment, 1, "Wrong answer on Compare-Command");
                        return (WRONG_ANSWER_COPY + GetAndReportErrorNumber(IspEnvironment,Answer));
                    }
                }
            }
    ...
    if (IspEnvironment->Verify)
        DebugPrintf(IspEnvironment, 2, "Download Finished and Verified correct... taking %d seconds", tDoneUpload - tStartUpload);
    else
        DebugPrintf(IspEnvironment, 2, "Download Finished... taking %d seconds", tDoneUpload - tStartUpload);

    //if (WaitForWatchDog)
    //{
    //    DebugPrintf(IspEnvironment, 2, "Wait for restart, in %d seconds from now\n", WatchDogSeconds - (tDoneUpload - tStartUpload));
    //}
    //else if(IspEnvironment->DoNotStart == 0)
    //{
    //    DebugPrintf(IspEnvironment, 2, "Now launching the brand new code\n");
    //    fflush(stdout);
