...
size_t DERLengthEncode(BufferedTransformation &bt, lword length)
{
  ...
  {
    bt.Put(CryptoPP::byte(BytePrecision(length) | 0x80));
    i++;
    for (int j=BytePrecision(length); j; --j)
    {
      bt.Put(CryptoPP::byte(length >> (j-1)*8));
      i++;
    }
  }
  return i;
...
/// ASN Strings
size_t DEREncodeOctetString(BufferedTransformation &bt, const CryptoPP::byte *str, size_t strLen)
{
  bt.Put(OCTET_STRING);
...
size_t DEREncodeTextString(BufferedTransformation &bt, const std::string &str, CryptoPP::byte asnTag)
{
  bt.Put(asnTag);
  size_t lengthBytes = DERLengthEncode(bt, str.size());
  bt.Put((const CryptoPP::byte *)str.data(), str.size());
  return 1+lengthBytes+str.size();
}

size_t BERDecodeTextString(BufferedTransformation &bt, std::string &str, CryptoPP::byte asnTag)
{
  CryptoPP::byte b;
...
/// ASN BitString
size_t DEREncodeBitString(BufferedTransformation &bt, const CryptoPP::byte *str, size_t strLen, unsigned int unusedBits)
{
  bt.Put(BIT_STRING);
  size_t lengthBytes = DERLengthEncode(bt, strLen+1);
  bt.Put((CryptoPP::byte)unusedBits);
  bt.Put(str, strLen);
...
void OID::EncodeValue(BufferedTransformation &bt, word32 v)
{
  for (unsigned int i=RoundUpToMultipleOf(STDMAX(7U,BitPrecision(v)), 7U)-7; i != 0; i-=7)
    bt.Put((CryptoPP::byte)(0x80 | ((v >> i) & 0x7f)));
  bt.Put((CryptoPP::byte)(v & 0x7f));
}
...
void OID::DEREncode(BufferedTransformation &bt) const
{
  ...
  ByteQueue temp;
  temp.Put(CryptoPP::byte(m_values[0] * 40 + m_values[1]));
...
void EncodedObjectFilter::Put(const CryptoPP::byte *inString, size_t length)
{
...
BERGeneralDecoder::BERGeneralDecoder(BufferedTransformation &inQueue, CryptoPP::byte asnTag)
  : m_inQueue(inQueue), m_finished(false)
{
  Init(asnTag);
}

BERGeneralDecoder::BERGeneralDecoder(BERGeneralDecoder &inQueue, CryptoPP::byte asnTag)
  : m_inQueue(inQueue), m_finished(false)
{
  Init(asnTag);
}

void BERGeneralDecoder::Init(CryptoPP::byte asnTag)
{
  CryptoPP::byte b;
...
CryptoPP::byte BERGeneralDecoder::PeekByte() const
{
  CryptoPP::byte b;
  if (!Peek(b))
    BERDecodeError();
...
void BERGeneralDecoder::CheckByte(CryptoPP::byte check)
{
  CryptoPP::byte b;
  if (!Get(b) || b != check)
    BERDecodeError();
...
DERGeneralEncoder::DERGeneralEncoder(BufferedTransformation &outQueue, CryptoPP::byte asnTag)
...
DERGeneralEncoder::DERGeneralEncoder(DERGeneralEncoder &outQueue, CryptoPP::byte asnTag)
  : ByteQueue(), m_outQueue(outQueue), m_finished(false), m_asnTag(asnTag)
{
}
