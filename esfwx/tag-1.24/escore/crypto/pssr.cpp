...
// more in dll.cpp
template<> const CryptoPP::byte EMSA2HashId<RIPEMD160>::id = 0x31;
template<> const CryptoPP::byte EMSA2HashId<RIPEMD128>::id = 0x32;
template<> const CryptoPP::byte EMSA2HashId<Whirlpool>::id = 0x37;
  ...
  CryptoPP::byte *representative, size_t representativeBitLength) const
{
  ...
  CryptoPP::byte *const h = representative + representativeByteLength - u - digestSize;
  ...
  CryptoPP::byte *xorStart = representative + representativeByteLength - u - digestSize - salt.size() - recoverableMessageLength - 1;
  xorStart[0] ^= 1;
  ...
  if (representativeBitLength % 8 != 0)
    representative[0] = (CryptoPP::byte)Crop(representative[0], representativeBitLength % 8);
}

DecodingResult PSSR_MEM_Base::RecoverMessageFromRepresentative(
  HashTransformation &hash, HashIdentifier hashIdentifier, bool messageEmpty,
  CryptoPP::byte *representative, size_t representativeBitLength,
  CryptoPP::byte *recoverableMessage) const
{
  ...
  const size_t digestSize = hash.DigestSize();
  const size_t saltSize = SaltLen(digestSize);
  const CryptoPP::byte *const h = representative + representativeByteLength - u - digestSize;
  ...
  CryptoPP::byte *salt = representative + representativeByteLength - u - digestSize - saltSize;
  CryptoPP::byte *M = std::find_if(representative, salt-1, std::bind2nd(std::not_equal_to<CryptoPP::byte>(), CryptoPP::byte(0)));
