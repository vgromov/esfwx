...
size_t NonblockingSource::GeneralPump2(
  ...
  unsigned long maxTime, bool checkDelimiter, CryptoPP::byte delimiter)
{
...
size_t NetworkSource::DoPump(lword &byteCount, bool blockingOutput, unsigned long maxTime, bool checkDelimiter, CryptoPP::byte delimiter)
{
...
size_t NetworkSink::Put2(const CryptoPP::byte *inString, size_t length, int messageEnd, bool blocking)
{
...
lword NetworkSink::DoFlush(unsigned long maxTime, size_t targetSize)
{
  ...
  while (true)
  {
    ...
    const CryptoPP::byte *block = m_buffer.Spy(contiguousSize);
