...
size_t FileStore::CopyRangeTo2(BufferedTransformation &target, lword &begin, lword end, const std::string &channel, bool blocking) const
{
  ...
  if (begin == 0 && end == 1)
  {
    ...
    {
      size_t blockedBytes = target.ChannelPut(channel, CryptoPP::byte(result), blocking);
...
size_t FileSink::Put2(const CryptoPP::byte *inString, size_t length, int messageEnd, bool blocking)
{
