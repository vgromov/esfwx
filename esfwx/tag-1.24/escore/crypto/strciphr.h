...
struct CRYPTOPP_DLL CRYPTOPP_NO_VTABLE AdditiveCipherAbstractPolicy
{
  ...
  virtual void WriteKeystream(CryptoPP::byte *keystream, size_t iterationCount)
  ...
  virtual void OperateKeystream(KeystreamOperation operation, CryptoPP::byte *output, const CryptoPP::byte *input, size_t iterationCount)
  ...
  virtual void CipherSetKey(const NameValuePairs &params, const CryptoPP::byte *key, size_t length) =0;

  //! \brief Resynchronize the cipher
  //! \param keystreamBuffer the keystream buffer
  //! \param iv a CryptoPP::byte array used to resynchronize the cipher
  //! \param length the size of the IV array
  virtual void CipherResynchronize(CryptoPP::byte *keystreamBuffer, const CryptoPP::byte *iv, size_t length)
...
struct CRYPTOPP_NO_VTABLE AdditiveCipherConcretePolicy : public BASE
{
  ...
  //!   which will be derived from GetBytesPerIteration().
  //! \sa CanOperateKeystream(), OperateKeystream(), WriteKeystream(), KeystreamOperation()
  virtual void OperateKeystream(KeystreamOperation operation, CryptoPP::byte *output, const CryptoPP::byte *input, size_t iterationCount) =0;
};
...
public:
  ...
  //! <pre>
  //!     size_t size = 30;
  //!     CryptoPP::byte plain[size] = "Do or do not; there is no try";
  //!     CryptoPP::byte cipher[size];
  //!     ...
  ...
  //! \param iv a CryptoPP::byte array used to resynchronize the cipher
  //! \param length the size of the IV array
  void Resynchronize(const CryptoPP::byte *iv, int length=-1);
...
protected:
  ...
  inline CryptoPP::byte * KeystreamBufferBegin() {return this->m_buffer.data();}
  inline CryptoPP::byte * KeystreamBufferEnd() {return (this->m_buffer.data() + this->m_buffer.size());}
...
public:
  ...
  //! \returns pointer to the first CryptoPP::byte of the feedback register
  virtual CryptoPP::byte * GetRegisterBegin() =0;
  ...
  virtual void Iterate(CryptoPP::byte *output, const CryptoPP::byte *input, CipherDir dir, size_t iterationCount)
  ...
  //! \param iv a CryptoPP::byte array used to resynchronize the cipher
  //! \param length the size of the IV array
  virtual void CipherResynchronize(const CryptoPP::byte *iv, size_t length)
...
struct CRYPTOPP_NO_VTABLE CFB_CipherConcretePolicy : public BASE
{
  ...
  {
    RegisterOutput(CryptoPP::byte *output, const CryptoPP::byte *input, CipherDir dir)
    ...
    CryptoPP::byte *m_output;
    const CryptoPP::byte *m_input;
    CipherDir m_dir;
  };
};
...
protected:
  virtual void CombineMessageAndShiftRegister(CryptoPP::byte *output, CryptoPP::byte *reg, const CryptoPP::byte *message, size_t length) =0;
...
{
  bool IsForwardTransformation() const {return true;}
  void CombineMessageAndShiftRegister(CryptoPP::byte *output, CryptoPP::byte *reg, const CryptoPP::byte *message, size_t length);
};
...
public:
  ...
  SymmetricCipherFinal(const CryptoPP::byte *key)
    {this->SetKey(key, this->DEFAULT_KEYLENGTH);}

  //! \brief Construct a stream cipher
  //! \param key a CryptoPP::byte array used to key the cipher
  //! \param length the size of the key array
  SymmetricCipherFinal(const CryptoPP::byte *key, size_t length)
    {this->SetKey(key, length);}

  //! \brief Construct a stream cipher
  //! \param key a CryptoPP::byte array used to key the cipher
  //! \param length the size of the key array
  //! \param iv a CryptoPP::byte array used as an initialization vector
  SymmetricCipherFinal(const CryptoPP::byte *key, size_t length, const CryptoPP::byte *iv)
