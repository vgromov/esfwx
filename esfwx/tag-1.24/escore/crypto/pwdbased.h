...
public:
  ...
  //! \brief Determines if the derivation function uses the purpose CryptoPP::byte
  //! \returns true if the derivation function uses the purpose CryptoPP::byte, false otherwise
  virtual bool UsesPurposeByte() const =0;

  //! \brief Derive key from the password
  //! \param derived the CryptoPP::byte buffer to receive the derived password
  //! \param derivedLen the size of the CryptoPP::byte buffer to receive the derived password
  //! \param purpose an octet indicating the purpose of the derivation
  //! \param password the CryptoPP::byte buffer with the password
  //! \param passwordLen the size of the password, in bytes
  //! \param salt the CryptoPP::byte buffer with the salt
  ...
  virtual unsigned int DeriveKey(CryptoPP::byte *derived, size_t derivedLen, CryptoPP::byte purpose, const CryptoPP::byte *password, size_t passwordLen, const CryptoPP::byte *salt, size_t saltLen, unsigned int iterations, double timeInSeconds=0) const =0;
};
...
public:
  ...
  // PKCS #5 says PBKDF1 should only take 8-CryptoPP::byte salts. This implementation allows salts of any length.
  unsigned int DeriveKey(CryptoPP::byte *derived, size_t derivedLen, CryptoPP::byte purpose, const CryptoPP::byte *password, size_t passwordLen, const CryptoPP::byte *salt, size_t saltLen, unsigned int iterations, double timeInSeconds=0) const;
};
...
template <class T>
unsigned int PKCS5_PBKDF1<T>::DeriveKey(CryptoPP::byte *derived, size_t derivedLen, CryptoPP::byte purpose, const CryptoPP::byte *password, size_t passwordLen, const CryptoPP::byte *salt, size_t saltLen, unsigned int iterations, double timeInSeconds) const
{
...
template <class T>
unsigned int PKCS5_PBKDF2_HMAC<T>::DeriveKey(CryptoPP::byte *derived, size_t derivedLen, CryptoPP::byte purpose, const CryptoPP::byte *password, size_t passwordLen, const CryptoPP::byte *salt, size_t saltLen, unsigned int iterations, double timeInSeconds) const
{
  CRYPTOPP_UNUSED(purpose);
  CRYPTOPP_ASSERT(derivedLen <= MaxDerivedKeyLength());
  CRYPTOPP_ASSERT(iterations > 0 || timeInSeconds > 0);
  ...
  while (derivedLen > 0)
  {
    ...
    for (j=0; j<4; j++)
    {
      CryptoPP::byte b = CryptoPP::byte(i >> ((3-j)*8));
      hmac.Update(&b, 1);
    }
    hmac.Final(buffer);
...
public:
  ...
  unsigned int DeriveKey(CryptoPP::byte *derived, size_t derivedLen, CryptoPP::byte purpose, const CryptoPP::byte *password, size_t passwordLen, const CryptoPP::byte *salt, size_t saltLen, unsigned int iterations, double timeInSeconds) const;
};

template <class T>
unsigned int PKCS12_PBKDF<T>::DeriveKey(CryptoPP::byte *derived, size_t derivedLen, CryptoPP::byte purpose, const CryptoPP::byte *password, size_t passwordLen, const CryptoPP::byte *salt, size_t saltLen, unsigned int iterations, double timeInSeconds) const
{
  CRYPTOPP_ASSERT(derivedLen <= MaxDerivedKeyLength());
  CRYPTOPP_ASSERT(iterations > 0 || timeInSeconds > 0);
  ...
  CryptoPP::byte *D = buffer, *S = buffer+DLen, *P = buffer+DLen+SLen, *I = S;
