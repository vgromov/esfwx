...
void P1363_MGF1KDF2_Common(HashTransformation &hash, CryptoPP::byte *output, size_t outputLength, const CryptoPP::byte *input, size_t inputLength, const CryptoPP::byte *derivationParams, size_t derivationParamsLength, bool mask, unsigned int counterStart)
{
  ArraySink *sink;
...
void TF_SignerBase::InputRecoverableMessage(PK_MessageAccumulator &messageAccumulator, const CryptoPP::byte *recoverableMessage, size_t recoverableMessageLength) const
{
...
size_t TF_SignerBase::SignAndRestart(RandomNumberGenerator &rng, PK_MessageAccumulator &messageAccumulator, CryptoPP::byte *signature, bool restart) const
{
...
void TF_VerifierBase::InputSignature(PK_MessageAccumulator &messageAccumulator, const CryptoPP::byte *signature, size_t signatureLength) const
{
  PK_MessageAccumulatorBase &ma = static_cast<PK_MessageAccumulatorBase &>(messageAccumulator);
  HashIdentifier id = GetHashIdentifier();
  const MessageEncodingInterface &encoding = GetMessageEncodingInterface();
...
DecodingResult TF_VerifierBase::RecoverAndRestart(CryptoPP::byte *recoveredMessage, PK_MessageAccumulator &messageAccumulator) const
{
  PK_MessageAccumulatorBase &ma = static_cast<PK_MessageAccumulatorBase &>(messageAccumulator);
  HashIdentifier id = GetHashIdentifier();
  const MessageEncodingInterface &encoding = GetMessageEncodingInterface();
...
DecodingResult TF_DecryptorBase::Decrypt(RandomNumberGenerator &rng, const CryptoPP::byte *ciphertext, size_t ciphertextLength, CryptoPP::byte *plaintext, const NameValuePairs &parameters) const
{
...
void TF_EncryptorBase::Encrypt(RandomNumberGenerator &rng, const CryptoPP::byte *plaintext, size_t plaintextLength, CryptoPP::byte *ciphertext, const NameValuePairs &parameters) const
{
