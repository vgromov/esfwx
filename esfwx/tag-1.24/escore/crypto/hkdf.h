...
public:
  ...
  virtual unsigned int DeriveKey(CryptoPP::byte *derived, size_t derivedLen, const CryptoPP::byte *secret, size_t secretLen, const CryptoPP::byte *salt, size_t saltLen, const CryptoPP::byte* info=NULL, size_t infoLen=0) const =0;
...
public:
  ...
  unsigned int DeriveKey(CryptoPP::byte *derived, size_t derivedLen, const CryptoPP::byte *secret, size_t secretLen, const CryptoPP::byte *salt, size_t saltLen, const CryptoPP::byte* info, size_t infoLen) const;
...
protected:
  ...
  typedef CryptoPP::byte NullVectorType[SALTSIZE];
...
template <class T>
unsigned int HKDF<T>::DeriveKey(CryptoPP::byte *derived, size_t derivedLen, const CryptoPP::byte *secret, size_t secretLen, const CryptoPP::byte *salt, size_t saltLen, const CryptoPP::byte* info, size_t infoLen) const
{
  ...
  HMAC<T> hmac;
  FixedSizeSecBlock<CryptoPP::byte, DIGEST_SIZE> prk, buffer;

  // Extract
  const CryptoPP::byte* key = (salt ? salt : GetNullVector());
