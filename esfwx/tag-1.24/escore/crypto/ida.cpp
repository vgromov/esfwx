...
void RawIDA::ChannelData(word32 channelId, const CryptoPP::byte *inString, size_t length, bool messageEnd)
{
...
size_t SecretSharing::Put2(const CryptoPP::byte *begin, size_t length, int messageEnd, bool blocking)
{
  if (!blocking)
...
size_t InformationDispersal::Put2(const CryptoPP::byte *begin, size_t length, int messageEnd, bool blocking)
{
  if (!blocking)
...
size_t PaddingRemover::Put2(const CryptoPP::byte *begin, size_t length, int messageEnd, bool blocking)
{
  if (!blocking)
    throw BlockingInputOnly("PaddingRemover");

  const CryptoPP::byte *const end = begin + length;

  if (m_possiblePadding)
  {
    size_t len = std::find_if(begin, end, std::bind2nd(std::not_equal_to<CryptoPP::byte>(), CryptoPP::byte(0))) - begin;
    m_zeroCount += len;
    begin += len;
    if (begin == end)
      return 0;
  ...
  typedef std::reverse_bidirectional_iterator<const CryptoPP::byte *, const CryptoPP::byte> RevIt;
#elif defined(_RWSTD_NO_CLASS_PARTIAL_SPEC)
  typedef std::reverse_iterator<const CryptoPP::byte *, std::random_access_iterator_tag, const CryptoPP::byte> RevIt;
#else
  typedef std::reverse_iterator<const CryptoPP::byte *> RevIt;
#endif
  const CryptoPP::byte *x = std::find_if(RevIt(end), RevIt(begin), std::bind2nd(std::not_equal_to<CryptoPP::byte>(), CryptoPP::byte(0))).base();
  if (x != begin && *(x-1) == 1)
  {
