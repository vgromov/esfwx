...
protected:
  ...
  CryptoPP::byte * AccessIpad() {return m_buf;}
  CryptoPP::byte * AccessOpad() {return m_buf + AccessHash().BlockSize();}
  CryptoPP::byte * AccessInnerHash() {return m_buf + 2*AccessHash().BlockSize();}
...
public:
  ...
  HMAC(const CryptoPP::byte *key, size_t length=HMAC_Base::DEFAULT_KEYLENGTH)
