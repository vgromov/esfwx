...
public:
  ...
  BaseN_Encoder(const CryptoPP::byte *alphabet, int log2base, BufferedTransformation *attachment=NULL, int padding=-1)
    : m_alphabet(NULL), m_padding(0), m_bitsPerChar(0)
    , m_outputBlockSize(0), m_bytePos(0), m_bitPos(0)
  {
    Detach(attachment);
    IsolatedInitialize(MakeParameters(Name::EncodingLookupArray(), alphabet)
      ...
      (Name::Pad(), padding != -1)
      (Name::PaddingByte(), CryptoPP::byte(padding)));
  }
...
private:
  const CryptoPP::byte *m_alphabet;
...
public:
  ...
  static void CRYPTOPP_API InitializeDecodingLookupArray(int *lookup, const CryptoPP::byte *alphabet, unsigned int base, bool caseInsensitive);
