...
struct SEAL_Gamma
{
  SEAL_Gamma(const CryptoPP::byte *key)
...
template <class B>
void SEAL_Policy<B>::CipherSetKey(const NameValuePairs &params, const CryptoPP::byte *key, size_t length)
{
...
template <class B>
void SEAL_Policy<B>::CipherResynchronize(CryptoPP::byte *keystreamBuffer, const CryptoPP::byte *IV, size_t length)
{
...
template <class B>
void SEAL_Policy<B>::OperateKeystream(KeystreamOperation operation, CryptoPP::byte *output, const CryptoPP::byte *input, size_t iterationCount)
{
  ...
  {
    #define Ttab(x) *(word32 *)(void*)((CryptoPP::byte *)m_T.begin()+x)
