...
public:
  ...
  void TruncatedFinal(CryptoPP::byte *mac, size_t size);
  unsigned int DigestSize() const {return DIGESTSIZE;}

private:
  CryptoPP::byte *GenerateSubKeys(const CryptoPP::byte *key, size_t keylength);
...
public:
  DMAC() {}
  DMAC(const CryptoPP::byte *key, size_t length=DMAC_Base<T>::DEFAULT_KEYLENGTH)
    {this->SetKey(key, length);}
};

template <class T>
void DMAC_Base<T>::UncheckedSetKey(const CryptoPP::byte *key, unsigned int length, const NameValuePairs &params)
{
...
template <class T>
void DMAC_Base<T>::Update(const CryptoPP::byte *input, size_t length)
{
...
template <class T>
void DMAC_Base<T>::TruncatedFinal(CryptoPP::byte *mac, size_t size)
{
  ThrowIfInvalidTruncatedSize(size);

  CryptoPP::byte pad[T::BLOCKSIZE];
  CryptoPP::byte padByte = CryptoPP::byte(T::BLOCKSIZE-m_counter);
...
template <class T>
CryptoPP::byte *DMAC_Base<T>::GenerateSubKeys(const CryptoPP::byte *key, size_t keylength)
{
