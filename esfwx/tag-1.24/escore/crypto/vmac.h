...
public:
  ...
  void Resynchronize(const CryptoPP::byte *nonce, int length=-1);
  void GetNextIV(RandomNumberGenerator &rng, CryptoPP::byte *IV);
  unsigned int DigestSize() const {return m_is128 ? 16 : 8;};
  void UncheckedSetKey(const CryptoPP::byte *userKey, unsigned int keylength, const NameValuePairs &params);
...
protected:
  ...
  CRYPTOPP_BLOCK_3(data, CryptoPP::byte, m_L1KeyLength)
  CRYPTOPP_BLOCK_4(l3Key, word64, 2*(m_is128+1))
  CRYPTOPP_BLOCK_5(nonce, CryptoPP::byte, IVSize())
  CRYPTOPP_BLOCK_6(pad, CryptoPP::byte, IVSize())
