...
const CryptoPP::byte * MessageQueue::Spy(size_t &contiguousSize) const
{
  const CryptoPP::byte *result = m_queue.Spy(contiguousSize);
...
size_t EqualityComparisonFilter::ChannelPut2(const std::string &channel, const CryptoPP::byte *inString, size_t length, int messageEnd, bool blocking)
{
  if (!blocking)
  ...
  else
  {
    ...
    {
      size_t len = length;
      const CryptoPP::byte *data = q2.Spy(len);
...
bool EqualityComparisonFilter::ChannelMessageSeriesEnd(const std::string &channel, int propagation, bool blocking)
{
  ...
  else
  {
    ...
    if (q2.AnyRetrievable() || q2.AnyMessages())
      ...
      return Output(2, (const CryptoPP::byte *)"\1", 1, 0, blocking) != 0;
    else
      q1.MessageSeriesEnd();
...
bool EqualityComparisonFilter::HandleMismatchDetected(bool blocking)
{
  ...
  return Output(1, (const CryptoPP::byte *)"\0", 1, 0, blocking) != 0;
}
