...
void TTMAC_Base::UncheckedSetKey(const CryptoPP::byte *userKey, unsigned int keylength, const NameValuePairs &)
{
...
void TTMAC_Base::TruncatedFinal(CryptoPP::byte *hash, size_t size)
{
  ...
  if (size != DIGESTSIZE)
  {
    switch (size)
    {
      ...
      default:
        throw InvalidArgument("TTMAC_Base: can't truncate a Two-Track-MAC 20 CryptoPP::byte digest to " + IntToString(size) + " bytes");
        break;
    }
  }
