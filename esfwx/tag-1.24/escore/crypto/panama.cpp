...
template <class B>
void Panama<B>::Iterate(size_t count, const word32 *p, CryptoPP::byte *output, const CryptoPP::byte *input, KeystreamOperation operation)
{
...
#define bPtr ((CryptoPP::byte *)(aPtr+20))
...
template <class B>
void PanamaHash<B>::TruncatedFinal(CryptoPP::byte *hash, size_t size)
{
...
template <class B>
void PanamaCipherPolicy<B>::CipherSetKey(const NameValuePairs &params, const CryptoPP::byte *key, size_t length)
{
...
template <class B>
void PanamaCipherPolicy<B>::CipherResynchronize(CryptoPP::byte *keystreamBuffer, const CryptoPP::byte *iv, size_t length)
{
...
template <class B>
void PanamaCipherPolicy<B>::OperateKeystream(KeystreamOperation operation, CryptoPP::byte *output, const CryptoPP::byte *input, size_t iterationCount)
{
#if (CRYPTOPP_BOOL_SSE2_ASM_AVAILABLE || defined(CRYPTOPP_X64_MASM_AVAILABLE)) && !defined(CRYPTOPP_DISABLE_PANAMA_ASM)
