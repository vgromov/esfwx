...
void AuthenticatedSymmetricCipherBase::AuthenticateData(const CryptoPP::byte *input, size_t len)
{
  unsigned int blockSize = AuthenticationBlockSize();
  unsigned int &num = m_bufferedDataLength;
  CryptoPP::byte* data = m_buffer.begin();
...
void AuthenticatedSymmetricCipherBase::SetKey(const CryptoPP::byte *userKey, size_t keylength, const NameValuePairs &params)
{
  ...
  size_t length;
  const CryptoPP::byte *iv = GetIVAndThrowIfInvalid(params, length);
  if (iv)
    Resynchronize(iv, (int)length);
}

void AuthenticatedSymmetricCipherBase::Resynchronize(const CryptoPP::byte *iv, int length)
{
...
void AuthenticatedSymmetricCipherBase::Update(const CryptoPP::byte *input, size_t length)
{
  if (length == 0)
    return;
...
void AuthenticatedSymmetricCipherBase::ProcessData(CryptoPP::byte *outString, const CryptoPP::byte *inString, size_t length)
{
...
void AuthenticatedSymmetricCipherBase::TruncatedFinal(CryptoPP::byte *mac, size_t macSize)
{
