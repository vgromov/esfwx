...
void NonblockingRng::GenerateBlock(CryptoPP::byte *output, size_t size)
{
#ifdef CRYPTOPP_WIN32_AVAILABLE
...
void BlockingRng::GenerateBlock(CryptoPP::byte *output, size_t size)
{
  while (size)
  {
...
void OS_GenerateRandomBlock(bool blocking, CryptoPP::byte *output, size_t size)
{
#ifdef NONBLOCKING_RNG_AVAILABLE
  if (blocking)
#endif
