...
void EAX_Base::SetKeyWithoutResync(const CryptoPP::byte *userKey, size_t keylength, const NameValuePairs &params)
{
  AccessMAC().SetKey(userKey, keylength, params);
  m_buffer.New(2*AccessMAC().TagSize());
}

void EAX_Base::Resync(const CryptoPP::byte *iv, size_t len)
{
...
size_t EAX_Base::AuthenticateBlocks(const CryptoPP::byte *data, size_t len)
{
...
void EAX_Base::AuthenticateLastFooterBlock(CryptoPP::byte *tag, size_t macSize)
{
  CRYPTOPP_ASSERT(m_bufferedDataLength == 0);
  MessageAuthenticationCode &mac = AccessMAC();
  unsigned int blockSize = mac.TagSize();
