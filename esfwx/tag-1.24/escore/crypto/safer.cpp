...
const CryptoPP::byte SAFER::Base::exp_tab[256] =
...
const CryptoPP::byte SAFER::Base::log_tab[256] =
...
void SAFER::Base::UncheckedSetKey(const CryptoPP::byte *userkey_1, unsigned int length, const NameValuePairs &params)
{
  ...
  const CryptoPP::byte *userkey_2 = length == 8 ? userkey_1 : userkey_1 + 8;
  keySchedule.New(1 + BLOCKSIZE * (1 + 2 * nof_rounds));

  unsigned int i, j;
  CryptoPP::byte *key = keySchedule;
...
typedef BlockGetAndPut<CryptoPP::byte, BigEndian> Block;

void SAFER::Enc::ProcessAndXorBlock(const CryptoPP::byte *inBlock, const CryptoPP::byte *xorBlock, CryptoPP::byte *outBlock) const
{
  CryptoPP::byte a, b, c, d, e, f, g, h, t;
  const CryptoPP::byte *key = keySchedule+1;
...
void SAFER::Dec::ProcessAndXorBlock(const CryptoPP::byte *inBlock, const CryptoPP::byte *xorBlock, CryptoPP::byte *outBlock) const
{
  CryptoPP::byte a, b, c, d, e, f, g, h, t;
  unsigned int round = keySchedule[0];
  const CryptoPP::byte *key = keySchedule + BLOCKSIZE * (1 + 2 * round) - 7;
