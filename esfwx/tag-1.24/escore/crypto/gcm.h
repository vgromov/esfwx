...
protected:
  ...
  void SetKeyWithoutResync(const CryptoPP::byte *userKey, size_t keylength, const NameValuePairs &params);
  void Resync(const CryptoPP::byte *iv, size_t len);
  size_t AuthenticateBlocks(const CryptoPP::byte *data, size_t len);
  void AuthenticateLastHeaderBlock();
  void AuthenticateLastConfidentialBlock();
  void AuthenticateLastFooterBlock(CryptoPP::byte *mac, size_t macSize);
  ...
  CryptoPP::byte *HashBuffer() {return m_buffer+REQUIRED_BLOCKSIZE;}
  CryptoPP::byte *HashKey() {return m_buffer+2*REQUIRED_BLOCKSIZE;}
  CryptoPP::byte *MulTable() {return m_buffer+3*REQUIRED_BLOCKSIZE;}
