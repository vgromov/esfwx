...
public:
  ...
  inline size_t Put(const CryptoPP::byte *begin, size_t length)
  {
  ...
  inline size_t Peek(CryptoPP::byte &outByte) const
  {
    if (m_tail==m_head)
      return 0;
  ...
  inline size_t Peek(CryptoPP::byte *target, size_t copyMax) const
  {
  ...
  inline size_t Get(CryptoPP::byte &outByte)
  {
  ...
  inline size_t Get(CryptoPP::byte *outString, size_t getMax)
  {
  ...
  inline CryptoPP::byte operator[](size_t i) const
  {
    return buf[m_head+i];
  }
...
size_t ByteQueue::Put2(const CryptoPP::byte *inString, size_t length, int messageEnd, bool blocking)
{
...
void ByteQueue::LazyPut(const CryptoPP::byte *inString, size_t size)
{
  if (m_lazyLength > 0)
    FinalizeLazyPut();
  ...
  {
    m_lazyString = const_cast<CryptoPP::byte *>(inString);
    m_lazyLength = size;
    m_lazyStringModifiable = false;
  }
}

void ByteQueue::LazyPutModifiable(CryptoPP::byte *inString, size_t size)
{
  if (m_lazyLength > 0)
    FinalizeLazyPut();
...
size_t ByteQueue::Get(CryptoPP::byte &outByte)
{
...
size_t ByteQueue::Get(CryptoPP::byte *outString, size_t getMax)
{
  ArraySink sink(outString, getMax);
  return (size_t)TransferTo(sink, getMax);
}

size_t ByteQueue::Peek(CryptoPP::byte &outByte) const
{
...
size_t ByteQueue::Peek(CryptoPP::byte *outString, size_t peekMax) const
{
...
void ByteQueue::Unget(CryptoPP::byte inByte)
{
  Unget(&inByte, 1);
}

void ByteQueue::Unget(const CryptoPP::byte *inString, size_t length)
{
...
const CryptoPP::byte * ByteQueue::Spy(size_t &contiguousSize) const
{
...
CryptoPP::byte * ByteQueue::CreatePutSpace(size_t &size)
{
  if (m_lazyLength > 0)
    FinalizeLazyPut();
...
CryptoPP::byte ByteQueue::operator[](lword i) const
{
  for (ByteQueueNode *current=m_head; current; current=current->next)
  {
...
size_t ByteQueue::Walker::Get(CryptoPP::byte &outByte)
{
  ArraySink sink(&outByte, 1);
  return (size_t)TransferTo(sink, 1);
}

size_t ByteQueue::Walker::Get(CryptoPP::byte *outString, size_t getMax)
{
  ArraySink sink(outString, getMax);
  return (size_t)TransferTo(sink, getMax);
}

size_t ByteQueue::Walker::Peek(CryptoPP::byte &outByte) const
{
  ArraySink sink(&outByte, 1);
  return (size_t)CopyTo(sink, 1);
}

size_t ByteQueue::Walker::Peek(CryptoPP::byte *outString, size_t peekMax) const
{
  ArraySink sink(outString, peekMax);
  return (size_t)CopyTo(sink, peekMax);
}
