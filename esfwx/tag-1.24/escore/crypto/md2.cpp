...
void MD2::Update(const CryptoPP::byte *buf, size_t len)
{
  static const CryptoPP::byte S[256] = {
  ...
  while (len)
    ...
    if (m_count==16)
    {
      ...
      for(i=0; i<18; i++)
      {
        ...
        t = CryptoPP::byte((t+i) & 0xFF);
      }
    }
    }
}

void MD2::TruncatedFinal(CryptoPP::byte *hash, size_t size)
{
  ThrowIfInvalidTruncatedSize(size);

  CryptoPP::byte padding[16];
  word32 padlen;
  unsigned int i;

  padlen= 16-m_count;
  for(i=0; i<padlen; i++) padding[i]=(CryptoPP::byte)padlen;
