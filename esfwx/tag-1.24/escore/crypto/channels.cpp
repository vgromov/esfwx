...
void MessageSwitch::Put(CryptoPP::byte inByte);
void MessageSwitch::Put(const CryptoPP::byte *inString, unsigned int length);

void MessageSwitch::Flush(bool completeFlush, int propagation=-1);
void MessageSwitch::MessageEnd(int propagation=-1);
void MessageSwitch::PutMessageEnd(const CryptoPP::byte *inString, unsigned int length, int propagation=-1);
...
size_t ChannelSwitch::ChannelPut2(const std::string &channel, const CryptoPP::byte *begin, size_t length, int messageEnd, bool blocking)
{
  if (m_blocked)
  {
    m_blocked = false;
...
CryptoPP::byte * ChannelSwitch::ChannelCreatePutSpace(const std::string &channel, size_t &size)
{
  m_it.Reset(channel);
  if (!m_it.End())
  {
...
size_t ChannelSwitch::ChannelPutModifiable2(const std::string &channel, CryptoPP::byte *inString, size_t length, int messageEnd, bool blocking)
{
