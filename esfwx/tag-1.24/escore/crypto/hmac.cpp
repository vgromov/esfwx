...
void HMAC_Base::UncheckedSetKey(const CryptoPP::byte *userKey, unsigned int keylength, const NameValuePairs &)
{
...
void HMAC_Base::Update(const CryptoPP::byte *input, size_t length)
{
  if (!m_innerHashKeyed)
    KeyInnerHash();
...
void HMAC_Base::TruncatedFinal(CryptoPP::byte *mac, size_t size)
{
