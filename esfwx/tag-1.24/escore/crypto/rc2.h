...
public:
  ...
  public:
    Encryption() {}
    Encryption(const CryptoPP::byte *key, size_t keyLen=DEFAULT_KEYLENGTH)
      {SetKey(key, keyLen);}
    Encryption(const CryptoPP::byte *key, size_t keyLen, int effectiveKeyLen)
  ...
  public:
    Decryption() {}
    Decryption(const CryptoPP::byte *key, size_t keyLen=DEFAULT_KEYLENGTH)
      {SetKey(key, keyLen);}
    Decryption(const CryptoPP::byte *key, size_t keyLen, int effectiveKeyLen)
      {SetKey(key, keyLen, MakeParameters("EffectiveKeyLength", effectiveKeyLen));}
  };
};
