...
void CCM_Base::SetKeyWithoutResync(const CryptoPP::byte *userKey, size_t keylength, const NameValuePairs &params)
{
...
void CCM_Base::Resync(const CryptoPP::byte *iv, size_t len)
{
  ...
  m_buffer[0] = CryptoPP::byte(m_L-1);  // flag
...
void CCM_Base::UncheckedSpecifyDataLengths(lword headerLength, lword messageLength, lword /*footerLength*/)
{
  ...
  CryptoPP::byte *cbcBuffer = CBC_Buffer();
  const BlockCipher &cipher = GetBlockCipher();

  cbcBuffer[0] = CryptoPP::byte(64*(headerLength>0) + 8*((m_digestSize-2)/2) + (m_L-1));  // flag
...
size_t CCM_Base::AuthenticateBlocks(const CryptoPP::byte *data, size_t len)
{
  CryptoPP::byte *cbcBuffer = CBC_Buffer();
  const BlockCipher &cipher = GetBlockCipher();
...
void CCM_Base::AuthenticateLastFooterBlock(CryptoPP::byte *mac, size_t macSize)
{
  m_ctr.Seek(0);
