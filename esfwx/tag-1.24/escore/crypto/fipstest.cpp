...
static const CryptoPP::byte s_moduleMac[CryptoPP::HMAC<CryptoPP::SHA1>::DIGESTSIZE] = CRYPTOPP_DUMMY_DLL_MAC;
...
const CryptoPP::byte * CRYPTOPP_API GetActualMacAndLocation(unsigned int &macSize, unsigned int &fileLocation)
{
  ...
  CIPHER *dummy = NULL)
{
  ...
  rng.Reseed((const CryptoPP::byte *)decodedKey.data(), decodedKey.size(), (const CryptoPP::byte *)decodedSeed.data(), (const CryptoPP::byte *)decodedDeterministicTimeVector.data());
  ...
  CIPHER *dummy = NULL)
{
  ...
  typename CIPHER::Encryption encryption((const CryptoPP::byte *)decodedKey.data(), decodedKey.size());
  typename CIPHER::Decryption decryption((const CryptoPP::byte *)decodedKey.data(), decodedKey.size());
...
void MAC_KnownAnswerTest(const char *key, const char *message, const char *digest, MAC *dummy = NULL)
{
  ...
  MAC mac((const CryptoPP::byte *)decodedKey.data(), decodedKey.size());
...
{
  CryptoPP::byte key[] = {0x47, 0x1E, 0x33, 0x96, 0x65, 0xB1, 0x6A, 0xED, 0x0B, 0xF8, 0x6B, 0xFD, 0x01, 0x65, 0x05, 0xCC};
  return new HMAC<SHA1>(key, sizeof(key));
}

bool IntegrityCheckModule(const char *moduleFilename, const CryptoPP::byte *expectedModuleMac, SecByteBlock *pActualMac, unsigned long *pMacFileLocation)
{
...
//  MeterFilter verifier(new FileSink("c:\\dt.tmp"));
  ...
  const CryptoPP::byte *memBase = (const CryptoPP::byte *)h;
  ...
  size_t checksumPos = (CryptoPP::byte *)&phnt->OptionalHeader.CheckSum - memBase;
  size_t checksumSize = sizeof(phnt->OptionalHeader.CheckSum);
  size_t certificateTableDirectoryPos = (CryptoPP::byte *)&phnt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_SECURITY] - memBase;
  ...
  while (nSections--)
  {
    ...
    case IMAGE_SCN_CNT_INITIALIZED_DATA | IMAGE_SCN_MEM_READ:
      ...
      const CryptoPP::byte *sectionMemStart = memBase + phs->VirtualAddress;
      unsigned int sectionFileStart = phs->PointerToRawData;
      size_t subSectionStart = 0, nextSubSectionStart;

      do
      {
        const CryptoPP::byte *subSectionMemStart = sectionMemStart + subSectionStart;
        ...
        for (unsigned int i=0; i<sizeof(entriesToReadFromDisk)/sizeof(entriesToReadFromDisk[0]); i++)
        {
          ...
          const CryptoPP::byte *entryMemStart = memBase + entry.VirtualAddress;
...
#if defined(_MSC_VER) && _MSC_VER >= 1400
        // first CryptoPP::byte of _CRT_DEBUGGER_HOOK gets modified in memory by the debugger invisibly, so read it from file
        if (IsDebuggerPresent())
        {
          if (subSectionMemStart <= (CryptoPP::byte *)&_CRT_DEBUGGER_HOOK && (CryptoPP::byte *)&_CRT_DEBUGGER_HOOK < subSectionMemStart + subSectionSize)
          {
            subSectionSize = (CryptoPP::byte *)&_CRT_DEBUGGER_HOOK - subSectionMemStart;
            nextSubSectionStart = (CryptoPP::byte *)&_CRT_DEBUGGER_HOOK - sectionMemStart + 1;
          }
        }
#endif
...
void DoPowerUpSelfTest(const char *moduleFilename, const CryptoPP::byte *expectedModuleMac)
{
