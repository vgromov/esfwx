...
void SosemanukPolicy::CipherSetKey(const NameValuePairs &params, const CryptoPP::byte *userKey, size_t keylen)
{
  CRYPTOPP_UNUSED(params);
  Serpent_KeySchedule(m_key, 24, userKey, keylen);
}

void SosemanukPolicy::CipherResynchronize(CryptoPP::byte *keystreamBuffer, const CryptoPP::byte *iv, size_t length)
{
...
extern "C" {
void Sosemanuk_OperateKeystream(size_t iterationCount, const CryptoPP::byte *input, CryptoPP::byte *output, word32 *state);
}
#endif

void SosemanukPolicy::OperateKeystream(KeystreamOperation operation, CryptoPP::byte *output, const CryptoPP::byte *input, size_t iterationCount)
{
...
#endif
  {
    ...
    FixedSizeAlignedSecBlock<CryptoPP::byte, 80*4*2+12*4+8*WORD_SZ> workspace;
  #endif
...
#define SSE2_STEP(i, j)  \
  ...
  {
#if (CRYPTOPP_BOOL_X86 || CRYPTOPP_BOOL_X32 || CRYPTOPP_BOOL_X64) && !defined(CRYPTOPP_DISABLE_SOSEMANUK_ASM)
#define MUL_A(x)    (x = rotlFixed(x, 8), x ^ s_sosemanukMulTables[CryptoPP::byte(x)])
#else
#define MUL_A(x)    (((x) << 8) ^ s_sosemanukMulTables[(x) >> 24])
#endif

#define DIV_A(x)    (((x) >> 8) ^ s_sosemanukMulTables[256 + CryptoPP::byte(x)])
