...
public:
  ...
  BlockCipherFinal(const CryptoPP::byte *key)
    {this->SetKey(key, this->DEFAULT_KEYLENGTH);}

  //! \brief Construct a BlockCipherFinal
  //! \param key a CryptoPP::byte array used to key the cipher
  //! \param length the length of the CryptoPP::byte array
  //! \details key must be at least DEFAULT_KEYLENGTH in length. Internally, the function calls
  //!    SimpleKeyingInterface::SetKey.
  BlockCipherFinal(const CryptoPP::byte *key, size_t length)
  ...
  BlockCipherFinal(const CryptoPP::byte *key, size_t length, unsigned int rounds)
...
public:
  ...
  //! \brief Construct a BlockCipherFinal
  //! \param key a CryptoPP::byte array used to key the algorithm
  //! \details key must be at least DEFAULT_KEYLENGTH in length. Internally, the function calls
  //!    SimpleKeyingInterface::SetKey.
  MessageAuthenticationCodeFinal(const CryptoPP::byte *key)
    {this->SetKey(key, this->DEFAULT_KEYLENGTH);}
  //! \brief Construct a BlockCipherFinal
  //! \param key a CryptoPP::byte array used to key the algorithm
  //! \param length the length of the CryptoPP::byte array
  //! \details key must be at least DEFAULT_KEYLENGTH in length. Internally, the function calls
  //!    SimpleKeyingInterface::SetKey.
  MessageAuthenticationCodeFinal(const CryptoPP::byte *key, size_t length)
    {this->SetKey(key, length);}
};
