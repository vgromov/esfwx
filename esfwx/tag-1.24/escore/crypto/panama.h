...
public:
  void Reset();
  void Iterate(size_t count, const word32 *p=NULL, CryptoPP::byte *output=NULL, const CryptoPP::byte *input=NULL, KeystreamOperation operation=WRITE_KEYSTREAM);
...
public:
   PanamaMAC() {}
  PanamaMAC(const CryptoPP::byte *key, unsigned int length)
