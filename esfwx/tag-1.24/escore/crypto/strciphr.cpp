...
template <class S>
void AdditiveCipherTemplate<S>::UncheckedSetKey(const CryptoPP::byte *key, unsigned int length, const NameValuePairs &params)
{
  ...
  {
    size_t ivLength;
    const CryptoPP::byte *iv = this->GetIVAndThrowIfInvalid(params, ivLength);
    policy.CipherResynchronize(m_buffer, iv, ivLength);
  }
}

template <class S>
void AdditiveCipherTemplate<S>::GenerateBlock(CryptoPP::byte *outString, size_t length)
{
  if (m_leftOver > 0)
  {
...
template <class S>
void AdditiveCipherTemplate<S>::ProcessData(CryptoPP::byte *outString, const CryptoPP::byte *inString, size_t length)
{
  if (m_leftOver > 0)
  {
    size_t len = STDMIN(m_leftOver, length);
...
template <class S>
void AdditiveCipherTemplate<S>::Resynchronize(const CryptoPP::byte *iv, int length)
{
  PolicyInterface &policy = this->AccessPolicy();
  m_leftOver = 0;
...
template <class BASE>
void CFB_CipherTemplate<BASE>::UncheckedSetKey(const CryptoPP::byte *key, unsigned int length, const NameValuePairs &params)
{
  PolicyInterface &policy = this->AccessPolicy();
  policy.CipherSetKey(params, key, length);
...
template <class BASE>
void CFB_CipherTemplate<BASE>::Resynchronize(const CryptoPP::byte *iv, int length)
{
  PolicyInterface &policy = this->AccessPolicy();
...
template <class BASE>
void CFB_CipherTemplate<BASE>::ProcessData(CryptoPP::byte *outString, const CryptoPP::byte *inString, size_t length)
{
  ...
  unsigned int bytesPerIteration = policy.GetBytesPerIteration();
  unsigned int alignment = policy.GetAlignment();
  CryptoPP::byte *reg = policy.GetRegisterBegin();
...
template <class BASE>
void CFB_EncryptionTemplate<BASE>::CombineMessageAndShiftRegister(CryptoPP::byte *output, CryptoPP::byte *reg, const CryptoPP::byte *message, size_t length)
{
...
template <class BASE>
void CFB_DecryptionTemplate<BASE>::CombineMessageAndShiftRegister(CryptoPP::byte *output, CryptoPP::byte *reg, const CryptoPP::byte *message, size_t length)
{
  for (unsigned int i=0; i<length; i++)
  {
    CryptoPP::byte b = message[i];
    output[i] = reg[i] ^ b;
    reg[i] = b;
  }
}
