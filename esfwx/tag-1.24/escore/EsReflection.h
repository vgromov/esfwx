...
{
public:
	EsMethodInfoKeyT(size_t paramsCount = 0, const EsString& name = EsString::null()) ES_NOTHROW :
  ...
  m_paramsCount(other.m_paramsCount), m_name(other.m_name)
	...
	inline size_t parametersCountGet() const ES_NOTHROW { return m_paramsCount; }
...
protected:
    ...
    ~SigStringsT() ES_NOTHROW;
		...
		const EsString& stringGet(int sig) const ES_NOTHROW;
...
public:
	EsMethodInfo(EsClassInfo& owner, int sig,	esU8 paramsCount, const EsString& name,
		const EsString& descr, EsMemberCallT method) ES_NOTHROW;
	EsMethodInfo(EsClassInfo& owner, int sig, esU8 paramsCount, const EsString& name,
		const EsString& descr, EsClassCallT classMethod, int tagClassCallInfo) ES_NOTHROW;
	EsMethodInfo(EsClassInfo& owner, int sig, const EsIID& iid, esU8 paramsCount,
  ...
  ~EsMethodInfo() ES_NOTHROW;
	...
	inline int signatureGet() const ES_NOTHROW { return m_signature; }
	inline int parametersCountGet() const ES_NOTHROW { return m_paramsCount; }
	inline const EsString& nameGet() const ES_NOTHROW { return m_name; }
  ...
  /// Access attribute storage
	...
	static inline const EsString& signatureStringGet(int sig) { return sigs().stringGet(sig); }
  ...
  static void fqNameParse(const EsString& fqName, EsString& className, bool& isClassMethod,
    EsString& methodName, int& paramCount, EsString& signature);
...
public:
	EsPropertyInfo(EsClassInfo& owner, const EsString& name, const EsString& label, const EsString& descr,
		const EsVariant& def, EsMemberCallT reader, int readerSig, EsMemberCallT writer, int writerSig,
