...
#define S(tag,name,encoding_index) { (int)(uintptr_t)&((struct stringpool2_t *)0)->stringpool_##tag, encoding_index },
#include "aliases2.h"
#undef S
};
#ifdef __GNUC__
...
#endif
const struct alias *
aliases2_lookup (ES_REGISTER const char *str)
{
...
iconv_t es_iconv_open (const char* tocode, const char* fromcode)
{
...
int es_iconv_close (iconv_t icd)
{
  conv_t cd = (conv_t) icd;
  free(cd);
  return 0;
...
int es_iconv_open_into (const char* tocode, const char* fromcode,
...
int es_iconvctl (iconv_t icd, int request, void* argument)
{
  conv_t cd = (conv_t) icd;
  switch (request) {
    case ICONV_TRIVIALP:
...
void es_iconvlist (int (*do_one) (unsigned int namescount,
...
const char * es_iconv_canonicalize (const char * name)
{
  const char* code;
