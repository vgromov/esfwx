...
///
class ESCOMM_CLASS EsBluetoothDeviceClass
{
public:
...
public:
  ...
  /// Decoding helpers
  ///
  /// Retrieve service class(es)
  ulong serviceGet() const
  { return (m_code & SERVICE_MASK) >> SERVICE_BIT_OFFSET; }
  /// Assign service class(es)
  void serviceSet(ulong svc)
  { m_code &= ~SERVICE_MASK; m_code |= (svc << SERVICE_BIT_OFFSET) & SERVICE_MASK; }

  /// Retrieve major device class
  ulong majorDeviceClassGet() const
  { return (m_code & MAJOR_MASK) >> MAJOR_BIT_OFFSET; }

  /// Assign major device class
  void majorDeviceClassSet(ulong mdc)
  { m_code &= ~MAJOR_MASK; m_code |= (mdc << MAJOR_BIT_OFFSET) & MAJOR_MASK; }

  /// Retrieve minor device class
  ulong minorDeviceClassGet() const
  { return (m_code & MINOR_MASK) >> MINOR_BIT_OFFSET; }

  /// Assign minor device class
  void minorDeviceClassSet(ulong mdc)
  { m_code &= ~MINOR_MASK; m_code |= (mdc << MINOR_BIT_OFFSET) & MINOR_MASK; }

  /// Retrieve COD format
  ulong formatGet() const
  { return (m_code & FORMAT_MASK) >> FORMAT_BIT_OFFSET; }

  /// Get device services string representation
  EsString serviceStringGet() const;

  /// Get major device code string representation
  EsString majorDeviceClassStringGet() const;

  /// Get minor device code string representation
  EsString minorDeviceClassStringGet() const;

  /// Convert code to string representation
  EsString asString() const;

  /// Type conversion
  operator ulong () const { return m_code; }

  /// Equality check operators
  bool operator==(const EsBluetoothDeviceClass& other) const { return m_code == other.m_code; }
  bool operator!=(const EsBluetoothDeviceClass& other) const { return m_code != other.m_code; }

  /// Minor class codes string conversion helpers
  static EsString comuterClassStringGet(ulong minor);
  static EsString phoneClassStringGet(ulong minor);
  static EsString lanClassStringGet(ulong minor);
  static EsString audioClassStringGet(ulong minor);
  static EsString peripheralClassStringGet(ulong minor);
  static EsString imagingClassStringGet(ulong minor);
  static EsString wearableClassStringGet(ulong minor);
  static EsString toyClassStringGet(ulong minor);
  static EsString healthClassStringGet(ulong minor);
...
///
class ESCOMM_CLASS EsBluetoothRadio
{
public:
  typedef std::shared_ptr<EsBluetoothRadio> Ptr;
...
public:
  ...
  /// Get radio device subversion
  ulong subversionGet() const { return m_subversion; }

  /// Get radio manufacturer code
  ulong manufacturerGet() const { return m_mfg; }

  /// Get manufacturer string
  EsString manufacturerStringGet() const;
...
typedef std::vector<EsBluetoothRadio::Ptr> EsBluetoothRadios;

/// Bluetooth radio enumerator class
///
class ESCOMM_CLASS EsBluetoothRadioEnumerator
{
public:
  /// Enumerator constructor.
  ///
  EsBluetoothRadioEnumerator() {}
  virtual ~EsBluetoothRadioEnumerator() {}
  ...
  const EsBluetoothRadios& resultGet() const { return m_infos; }
...
protected:
  ...
  ///
  virtual bool onRadioEnumerated(const EsBluetoothRadio::Ptr& info);

protected:
  /// Data members
  ///
  /// Enumerated infos
  EsBluetoothRadios m_infos;

private:
  /// Prohibited functionality
  EsBluetoothRadioEnumerator(const EsBluetoothRadioEnumerator&);
  EsBluetoothRadioEnumerator& operator=(const EsBluetoothRadioEnumerator&);
};
