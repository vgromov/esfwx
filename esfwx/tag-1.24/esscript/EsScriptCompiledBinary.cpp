...
void EsScriptCompiledBinaryWriter::stringWrite(const EsString& str)
{
	...
	checkReserveEnough(ustr.size()+4);
	esU32 tmp = ustr.size();
...
void EsScriptCompiledBinaryWriter::binaryLinksWrite()
{
	...
	esU32 tmp = m_machine.m_links.size();
	m_buff.append(4, reinterpret_cast<const esU8*>(&tmp));
...
void EsScriptCompiledBinaryWriter::codeSectionsWrite(const EsScriptMethodMapPtr& methods, int codeSectionTag /*= binaryTypeCodeSections*/)
{
	...
	if( methods )
		cnt = methods->size();
	m_buff.append(4, reinterpret_cast<const esU8*>(&cnt));
	if( methods )
	{
...
void EsScriptCompiledBinaryWriter::metaclassesWrite()
{
	...
	for( size_t idx = 0; idx < m_machine.m_metaclassesMap.countGet(); ++idx )
	{
...
void EsScriptCompiledBinaryWriter::enumWrite(const EsEnumerationIntf::Ptr& enu)
{
	...
	for(size_t idx = 0; idx < tmp; ++ idx)
		enumItemWrite(syms[idx], vals.itemGet(idx), labels[idx]);
}
//---------------------------------------------------------------------------
...
void EsScriptCompiledBinaryWriter::enumsWrite()
{
	...
	for( size_t idx = 0; idx < tmp; ++idx )
		enumWrite( m_machine.m_enumsMap.valueGet(idx).asExistingObject() );
}
//---------------------------------------------------------------------------
