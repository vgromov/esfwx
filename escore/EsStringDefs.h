...
# define _(x)                   EsStringI18n::translate( esT(x) )
#else
...
// Interpret parameter as variant initialized with ES_CHAR pointer
//
#define esVC(x) EsVariant((x), EsVariant::ACCEPT_STRING)

// Interpret parameter as variant initialized with object which has c_str service
//
#define esVS(x) EsVariant((x).c_str(), EsVariant::ACCEPT_STRING)
