...
class EsProgressMonitorTaskCtorAcc : public EsProgressMonitorTask
{
public:
  EsProgressMonitorTaskCtorAcc(const EsString& id, const EsString& text,  const EsVariant& range, const EsVariant& val) :
...
EsProgressMonitorTaskIntf::Ptr EsProgressMonitorTask::create(const EsString& id, const EsString& text,
  const EsVariant& range, const EsVariant& val)
{
  std::unique_ptr<EsProgressMonitorTaskCtorAcc> t = ES_MAKE_UNIQUE(
    EsProgressMonitorTaskCtorAcc,
    id,
    text, 
    range, 
    val
  ...
  EsProgressMonitorTask* child)
{
  ...
  EsBaseIntf::Ptr childPtr = child->asWeakReference();
  m_children.itemAdd(
    child->m_id,
    childPtr
  );
...
class EsProgressMonitorCtorAcc : public EsProgressMonitor
{
public:
  EsProgressMonitorCtorAcc(const EsString& id) :
  EsProgressMonitor(id)
  {}
};
//---------------------------------------------------------------------------
...
{
  std::unique_ptr<EsProgressMonitorCtorAcc> m = ES_MAKE_UNIQUE(
    EsProgressMonitorCtorAcc,
    id
  );
  ES_ASSERT(m);
  ...
  EsProgressMonitorTask* child)
{
  ...
  EsVariant::Array payload = {
    ...
    child->m_text,
    child->m_state,
    child->internalProgressTextGet(),
    (
      EsProgressMonitorTask::statePulsing == child->m_state ?
        EsVariant::null() :
        EsVariant(child->m_completion)
    )
  };
