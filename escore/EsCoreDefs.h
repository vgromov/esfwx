...
///
/// ES_PACKAGE should be defined at project level if we're targeting ECC's bpl project
///
#if defined(ES_PACKAGE) || defined(ES_USE_DLLS)
# define ES_DYNAMIC_LINKAGE
#endif

// ES_DYNAMIC_LINKAGE override for static package linkage under Borland toolchain
#if (ES_COMPILER_VENDOR == ES_COMPILER_VENDOR_BORLAND) && defined(ES_PACKAGE)
...
#   if !defined(ES_CLANG) && defined(ES_PACKAGE)
#     pragma warn -8118
...
private:\
  ClassName(const ClassName&) ES_REMOVEDECL; \
  ClassName& operator=(const ClassName&) ES_REMOVEDECL

#define ES_NO_DEFAULT_CTR(ClassName) \
private:\
  ClassName() ES_REMOVEDECL
...
//
#if (__cplusplus > 201103L) && (ES_COMPILER_VENDOR == ES_COMPILER_VENDOR_MS)
...
#if defined(ES_DEBUG) && !defined(ES_USE_MEMLEAK_TRACE)
# define ES_USE_MEMLEAK_TRACE
#endif

// Memory leak detection
#if defined(ES_DEBUG) && defined(ES_USE_MEMLEAK_TRACE) && (ES_OS == ES_OS_WINDOWS)
