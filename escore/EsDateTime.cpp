...
{
  std::unique_ptr<EsTimeSpan> p = ES_MAKE_UNIQUE(
    EsTimeSpan,
    dt
  );
  ES_ASSERT(p);
...
void EsTimeSpan::customCopyFrom(const EsReflectedClassIntf::Ptr& src)
{
  ES_ASSERT(src);

  const EsTimeSpan* ptr = ES_INTFPTR_TO_OBJECTPTR<EsTimeSpan>(src);
  ES_ASSERT(ptr);
...
esDT EsTimeSpan::fromVariant(const EsVariant& var)
{
  if( var.isObject() )
  {
    ...
    else if( var.hasProperty(esT("value")) )
    {
      const EsVariant& val = var.propertyGet("value");
      return val.asLLong();
    }

    EsException::Throw(
      esT("Could not convert variant of type %s to EsTimeSpan value"),
      var.kindGet()
    );
  }
  else
...
EsTimeSpan& EsTimeSpan::operator= (const EsTimeSpan& src)
{
  esDT other = static_cast<esDT>(src);
...
{
  std::unique_ptr<EsDateTime> p = ES_MAKE_UNIQUE(
    EsDateTime,
    dt
  );
  ES_ASSERT(p);
...
void EsDateTime::compose(ulong hour, ulong minute, ulong second, ulong millisecond)
{
  compose(
    DT_YEAR0,
    1,
...
esDT EsDateTime::fromVariant(const EsVariant& var)
{
  if( var.isObject() )
  {
    ...
    EsException::Throw(
      esT("Could not convert variant of type %s to esDT value"),
      var.kindGet()
    );
  }
  else
...
EsDateTime& EsDateTime::operator= (const EsDateTime& src)
{
  esDT other = static_cast<esDT>(src);
  if(m_dt != other)
  {
...
void EsDateTime::customCopyFrom(const EsReflectedClassIntf::Ptr& src)
{
  ES_ASSERT(src);

  const EsDateTime* ptr = ES_INTFPTR_TO_OBJECTPTR<EsDateTime>(src);
  ES_ASSERT(ptr);
