...
class EsAttributesCtorAcc : public EsAttributes
{
public:
  EsAttributesCtorAcc(const EsString& ownerName, bool interlocked) :
  EsAttributes(
    ownerName,
    interlocked
  )
...
EsAttributes::EsAttributes(const EsString& ownerName, bool interlocked) ES_NOTHROW :
m_contents(
  interlocked ?
...
{
  std::unique_ptr<EsAttributesCtorAcc> ptr = ES_MAKE_UNIQUE(
    EsAttributesCtorAcc,
    ownerName,
    interlocked
  );
...
//
EsMethodInfo::SigStringsT::~SigStringsT()
{
  m_sigs.clear();
}
//---------------------------------------------------------------------------
