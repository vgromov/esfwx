...
/// Reflected class declaration macros
///

#define ES_THISBASE_CAST  asBaseIntf()->implementorGet()

namespace EsReflection
{
// SFINAE checker for customCopyFrom member function
template<typename ClassT>
struct hasCustomCopyFrom
{
private:
  using yes = std::true_type;
  using no = std::false_type;
  
  template<typename InClassT>
  static yes has( decltype(&InClassT::customCopyFrom) );
...
public:
  enum { 
    value = std::is_same<decltype(has<ClassT>(nullptr)), yes>::value 
  };
};

namespace Internal
{
  /// Default Reflected copy implementation
  template <typename ThisT, typename std::enable_if< 0 == hasCustomCopyFrom<ThisT>::value, ThisT >::type* = nullptr>
  void copyFromOther(ThisT& to, const EsReflectedClassIntf::Ptr& from)
  {
    // Default implementation copies property-by-property
    const EsStringArray& propNames = to.persistentPropertyNamesGet();
    ...
    for(size_t idx = 0; idx < propNames.size(); ++idx) 
    {
      ...
      to.propertySet(
        prop, 
        from->propertyGet(prop)
      );
    }
  }

  /// Custom Reflected copy implementation
  template <typename ThisT, typename std::enable_if< 1 == hasCustomCopyFrom<ThisT>::value, ThisT >::type* = nullptr >
  void copyFromOther(ThisT& to, const EsReflectedClassIntf::Ptr& from)
  {
    to.customCopyFrom( from );
  }
}

/// Reflected copy implementation
template <typename ThisT>
void copyFromOther(ThisT& _this, cr_EsBaseIntfPtr other)
{
  EsReflectedClassIntf::Ptr obj = other; 
  ES_ASSERT(obj);

  if( !_this.isKindOf(obj) ) 
    EsExceptionThrow(
      esT("%s: Could not copy from object of different type %s"),
      _this.typeNameGet(),
      obj->typeNameGet()
    );

  Internal::copyFromOther<ThisT>(
    _this,
    obj
  );
}
...
public: \
  ...
  ES_DECL_REFLECTED_INTF_METHOD1(void, copyFrom, cr_EsBaseIntfPtr) ES_OVERRIDE { EsReflection::copyFromOther<ThisClassT>(*this, p1); } \
...
struct EsClassAttrInitializer
{
  ...
  ES_NO_DEFAULT_CTR(EsClassAttrInitializer);
  ES_NON_COPYABLE(EsClassAttrInitializer);
};
...
class ExportSpec NamespaceName { \
  ES_NO_DEFAULT_CTR(NamespaceName); \
  ES_NON_COPYABLE(NamespaceName); \
public: \
  static const EsClassInfo& classInfoGetStatic() ES_NOTHROW;
