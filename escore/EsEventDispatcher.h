...
protected:
  ...
  ES_NON_COPYABLE(EsEventDispatcher);

  friend class EsEventSubscriber;
};

/// Event subscriber base implementation
///
...
protected:
  virtual void onActivate() ES_NOTHROW {}
...
protected:
  ...
  ES_NO_DEFAULT_CTR(EsEventSubscriber);
  ES_NON_COPYABLE(EsEventSubscriber);
...
protected:
  virtual void onActivate() ES_NOTHROW ES_OVERRIDE { reset(); }

protected:
  EsSemaphore m_sem;
  EsEventQueue m_queue;

  ES_NON_COPYABLE(EsEventSubscriberAsync);
};
