...
ES_DECL_BASE_CLASS_INFO_BEGIN(EsUUID, NO_CLASS_DESCR)
  ...
  ES_DECL_REFLECTED_METHOD_INFO_STD(EsUUID, asString, EsString_CallConst, NO_METHOD_DESCR)
...
{
  std::unique_ptr<EsUUID> p = ES_MAKE_UNIQUE(
    EsUUID,
    uuid
  );
  ES_ASSERT(p);
...
GUID EsUUID::fromVariant(const EsVariant& var)
{
  ...
  {
    EsUUID* uuid = ES_INTFPTR_TO_OBJECTPTR<EsUUID>(
      var.asExistingObject()
    );
    ES_ASSERT(uuid);
...
void EsUUID::customCopyFrom(const EsReflectedClassIntf::Ptr& src)
{
  ES_ASSERT(src);

  const EsUUID* ptr = ES_INTFPTR_TO_OBJECTPTR<EsUUID>(src);
  ES_ASSERT(ptr);
