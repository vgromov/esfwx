...
public:
  ...
  iterator erase(iterator first, iterator last) { return m_bb.erase(first, last); }
  ...
  inline void append(const EsBinBuffer& src) { m_bb.append(src.m_bb); }
  ...
  inline size_type size() const ES_NOTHROW { return m_bb.size(); }
  ...
  // Direct access
  inline EsBinBuffer::pointer data() ES_NOTHROW { return m_bb.data(); }
  inline EsBinBuffer::const_pointer data() const ES_NOTHROW { return m_bb.data(); }
