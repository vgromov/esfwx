...
ES_DECL_BASE_CLASS_INFO_BEGIN(EsAssocContainer, NO_CLASS_DESCR)
  ...
  // reflected methods
  //
  ES_DECL_REFLECTED_METHOD_INFO_STD( EsAssocContainer, clone, EsVariant_CallConst, NO_METHOD_DESCR)
  ES_DECL_REFLECTED_METHOD_INFO_STD( EsAssocContainer, clone, EsVariant_CallConst_cr_EsVariant, NO_METHOD_DESCR)
...
{
  std::unique_ptr<EsAssocContainer> tmp = ES_MAKE_UNIQUE(
    EsAssocContainer,
    owner
  );
  ES_ASSERT(tmp);
...
EsVariant EsAssocContainer::clone(const EsVariant& factory) const
{
...
EsVariant EsAssocContainer::clone() const
{
  return clone( EsVariant::null() );
}
//---------------------------------------------------------------------------
...
void EsAssocContainer::keyCheck(const EsVariant& key) const
{
  if( key.isEmpty() )
    EsException::Throw(
      esT("EsAssocContainer::keyCheck: Item key must not be empty")
    );
  ...
  if( key.isObject() )
  {
    ...
    if( !obj->hasMethod( methodKey ) )
      EsException::Throw(
        esT("EsAssocContainer::keyCheck: Key object %s must implement '%s' service"),
        obj->typeNameGet(),
        methodKey.asString()
      );
    ...
    if( obj->isKindOf(esT("EsScriptObject")) &&
      ...
      EsException::Throw(
        esT("EsAssocContainer::keyCheck: Key object %s must be root"),
        obj->typeNameGet()
      );
  }
  else if( !(key.isNumeric() || EsVariant::VAR_STRING == key.typeGet()) )
    EsException::Throw(
      esT("EsAssocContainer::keyCheck: Key must be either numeric, string, or existing object")
    );
}
//---------------------------------------------------------------------------
...
void EsAssocContainer::customCopyFrom(const EsReflectedClassIntf::Ptr& src)
{
  ES_ASSERT(src);
  clear();

  const EsAssocContainer* ptr = ES_INTFPTR_TO_OBJECTPTR<EsAssocContainer>(src);
  ES_ASSERT(ptr);

  // In our implementation, owner, if set, provides a metaclass factory
  // for content streaming operations
  //
  m_owner = ptr->ownerGet();
