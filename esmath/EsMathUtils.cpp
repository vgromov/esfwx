...
ES_DECL_REFLECTED_SERVICES_INFO_BEGIN(EsMath, NO_CLASS_DESCR)
  ...
  ES_DECL_REFLECTED_CLASS_METHOD_INFO_STD(EsMath, accumulate, EsVariant_ClassCall_cr_EsVariant, NO_METHOD_DESCR)
...
EsVariant EsMath::accumulate(const EsVariant& in)
{
  double result = 0;
  const EsMathArrayReal* sp = ES_VARIANT_TO_OBJECTPTR<EsMathArrayReal>(in);
  if(sp)
    result = sp->get_total();
  else if(in.isCollection())
  {
    const EsVariant::Array& avals = in.asVariantCollection();
    for(auto const& item : avals)
