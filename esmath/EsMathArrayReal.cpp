...
ES_DECL_BASE_CLASS_INFO_BEGIN(EsMathArrayReal, NO_CLASS_DESCR)
  ...
  ES_DECL_REFLECTED_METHOD_INFO_STD(      EsMathArrayReal, minimaxReset, void_Call, NO_METHOD_DESCR)
...
void EsMathArrayReal::itemSet(ulong idx, double val)
{
  ...
  if(!get_dirty()) //< Fast inplace update, without total recalc
  {
    double min = itemGet(m_minIdx);
    double max = itemGet(m_maxIdx);
    if(min > val)
      m_minIdx = idx;
...
void EsMathArrayReal::minimaxReset()
{
  m_total = 0;
  m_minIdx = m_maxIdx = 0;
  m_needRecalc = true;
...
{
  std::unique_ptr<EsMathArrayReal> p = ES_MAKE_UNIQUE( 
    EsMathArrayReal,
    *this
  );
  ES_ASSERT(p);
...
{
  std::unique_ptr<EsMathArrayReal> p = ES_MAKE_UNIQUE( EsMathArrayReal );
  ES_ASSERT(p);
